...
function getAgentdocEditorApi(): Window['agentdoc'] | null {
  if (typeof window === 'undefined') return null;
  return window.agentdoc ?? null;
}
...
class MarkPopoverController {
  ...
  private renderThread(mark: Mark): void {
    ...
    if (canComment) {
      ...
      const reply = () => {
        ...
        if (!text) return;
        const agentdoc = getAgentdocEditorApi();
        const created = agentdoc?.markReply
          ? agentdoc.markReply(mark.id, getCurrentActor(), text)
      ...
      installTouchSafeButton(resolveButton, () => {
        const agentdoc = getAgentdocEditorApi();
        if (resolved) {
          if (agentdoc?.markUnresolve) {
            agentdoc.markUnresolve(mark.id);
          } else {
            unresolveComment(this.view, mark.id);
          }
          this.openForMark(mark.id);
        } else {
          if (agentdoc?.markResolve) {
            agentdoc.markResolve(mark.id);
          } else {
      ...
      installTouchSafeButton(deleteButton, () => {
        const agentdoc = getAgentdocEditorApi();
        if (agentdoc?.markDeleteThread) {
          agentdoc.markDeleteThread(mark.id);
        } else {
  ...
  private renderSuggestion(mark: Mark): void {
    ...
    installTouchSafeButton(applyButton, () => {
      if (!canEdit) return;
      const agentdoc = getAgentdocEditorApi();
      if (agentdoc?.markAccept) {
        agentdoc.markAccept(mark.id);
      } else {
    ...
    installTouchSafeButton(rejectButton, () => {
      if (!canEdit) return;
      const agentdoc = getAgentdocEditorApi();
      if (agentdoc?.markReject) {
        agentdoc.markReject(mark.id);
      } else {
