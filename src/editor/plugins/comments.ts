...
/**
 * Pattern to detect @agentdoc mentions
 */
const AGENTDOC_MENTION_PATTERN = /@agentdoc\b/i;

/**
 * Check if text contains @agentdoc mention
 */
export function hasAgentdocMention(text: string): boolean {
  return AGENTDOC_MENTION_PATTERN.test(text);
}
...
export interface CommentEventCallbacks {
  ...
  onAgentdocMentioned?: (comment: Comment, allComments: Comment[]) => void;
}
...
function emitCommentCreated(comment: Comment, allComments: Comment[]): void {
  ...
  // Check for @agentdoc mention
  if (hasAgentdocMention(comment.text)) {
    commentEventCallbacks.onAgentdocMentioned?.(comment, allComments);
  }
}
...
function emitCommentReplied(comment: Comment, reply: CommentReply, allComments: Comment[]): void {
  ...
  // Check for @agentdoc mention in reply
  if (hasAgentdocMention(reply.text)) {
    commentEventCallbacks.onAgentdocMentioned?.(comment, allComments);
  }
}
