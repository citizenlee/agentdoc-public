 ...
 * Metadata (comment text, suggestion content/status) lives in the AGENTDOC block.
 */
...
const MARK_TYPE_NAMES = {
  suggestion: 'agentdocSuggestion',
  comment: 'agentdocComment',
  flagged: 'agentdocFlagged',
  approved: 'agentdocApproved',
  authored: 'agentdocAuthored',
};
...
function getHydrationDocumentId(): string | null {
  ...
  const config = (window as Window & { __AGENTDOC_CONFIG__?: { documentId?: string } }).__AGENTDOC_CONFIG__;
...
function getAgentdocAnchorIds(doc: ProseMirrorNode): Map<string, { kind: MarkKind; by: string }> {
...
function normalizeMetadata(
  ...
  const ids = getAgentdocAnchorIds(doc);
  let changed = false;
...
function buildMetadataSnapshot(
  ...
  const anchoredIds = getAgentdocAnchorIds(state.doc);
...
export function applyRemoteMarks(
  ...
  const existingIds = getAgentdocAnchorIds(view.state.doc);
...
function createDecorations(
  ...
  for (const mark of resolved) {
    ...
    if (style) {
      ...
      const glowClass = markAge < GLOW_DURATION_MS ? 'agentdoc-mark-new' : '';
...
function injectGlowStyles(): void {
  ...
  style.id = 'agentdoc-mark-glow-styles';
  style.textContent = `
    /* Glow animation for newly-created suggestion marks */
    @keyframes agentdoc-change-glow {
    ...
    .agentdoc-mark-new { animation: agentdoc-change-glow 2s ease-out forwards; }

    /* Delete glow */
    .mark-delete.agentdoc-mark-new {
      animation-name: agentdoc-delete-glow;
    }
    @keyframes agentdoc-delete-glow {
      0% { box-shadow: 0 0 8px rgba(239, 68, 68, 0.6); }
      100% { box-shadow: none; }
    }

    /* Refresh transition animations */
    .agentdoc-refreshing { opacity: 0.3; transition: opacity 200ms ease-out; }
    .agentdoc-refreshed { opacity: 1; transition: opacity 300ms ease-in; }

    /* External change toast notification */
    .agentdoc-external-change-toast {
      position: fixed;
      top: 12px;
      right: 12px;
      z-index: 10000;
      ...
      max-width: 340px;
      animation: agentdoc-toast-slide-in 250ms ease-out;
    }
    .agentdoc-share-welcome-toast {
    ...
    @keyframes agentdoc-toast-slide-in {
      from { transform: translateY(-12px); opacity: 0; }
      to { transform: translateY(0); opacity: 1; }
    }
    @keyframes agentdoc-toast-slide-in-centered {
      from { transform: translate(-50%, -12px); opacity: 0; }
      to { transform: translate(-50%, 0); opacity: 1; }
    }
    .agentdoc-toast-content {
      display: flex;
      flex-direction: column;
      gap: 10px;
    }
    .agentdoc-toast-content--welcome {
      position: relative;
      padding-right: 20px;
    }
    .agentdoc-toast-message {
      font-weight: 600;
      font-size: 13px;
      line-height: 1.3;
    }
    .agentdoc-toast-submessage {
      font-size: 12px;
      color: #4b5563;
      line-height: 1.35;
    }
    @media (prefers-color-scheme: dark) {
      .agentdoc-toast-submessage { color: #cbd5f5; }
    }
    .agentdoc-toast-close {
      position: absolute;
      top: -2px;
      right: -4px;
      border: none;
    ...
    .agentdoc-toast-close:hover {
      opacity: 0.9;
      background: rgba(0, 0, 0, 0.06);
    }
    .agentdoc-toast-close:active {
      opacity: 1;
    }
    @media (max-width: 760px) {
      .agentdoc-share-welcome-toast {
        ...
        border-radius: 12px;
        padding: 10px 12px;
        animation: agentdoc-toast-slide-in-centered 250ms ease-out;
      }
      .agentdoc-share-welcome-toast .agentdoc-toast-content--welcome {
        padding-right: 24px;
      }
      .agentdoc-share-welcome-toast .agentdoc-toast-message {
        font-size: 12px;
        line-height: 1.35;
        overflow-wrap: anywhere;
      }
      .agentdoc-share-welcome-toast .agentdoc-toast-close {
        top: -1px;
        right: -2px;
      }
    }
    .agentdoc-toast-actions {
      display: flex;
      gap: 6px;
    }
    .agentdoc-toast-btn {
    ...
    .agentdoc-toast-primary {
      background: #111827;
      color: #fff;
    }
    .agentdoc-toast-secondary {
      background: transparent;
      color: #111827;
    ...
    .agentdoc-toast-review {
      background: #007AFF;
      color: white;
    }
    .agentdoc-toast-accept-all {
      background: #34C759;
      color: white;
    }
    .agentdoc-toast-reject-all {
      background: transparent;
      color: #FF3B30;
      border: 1px solid rgba(255, 59, 48, 0.3);
    }
    .agentdoc-toast-btn:hover { opacity: 0.85; }
    .agentdoc-toast-btn:active { opacity: 0.7; }

    /* Refresh banner */
    .agentdoc-refresh-banner {
      display: flex;
      align-items: center;
      ...
      animation: agentdoc-banner-slide-down 250ms ease-out;
    }
    @media (prefers-color-scheme: dark) {
      .agentdoc-refresh-banner {
    ...
    @keyframes agentdoc-banner-slide-down {
      from { transform: translateY(-8px); opacity: 0; }
      to { transform: translateY(0); opacity: 1; }
    }
    .agentdoc-refresh-banner-info {
      display: flex;
      flex-direction: column;
      gap: 2px;
    }
    .agentdoc-refresh-banner-title {
      font-weight: 600;
      font-size: 13px;
    }
    .agentdoc-refresh-banner-summary {
      font-size: 12px;
      color: #555;
      font-weight: 400;
    }
    @media (prefers-color-scheme: dark) {
      .agentdoc-refresh-banner-summary { color: #a0aec0; }
    }
    .agentdoc-refresh-banner-actions {
      display: flex;
      gap: 8px;
      align-items: center;
      flex-shrink: 0;
    }
    .agentdoc-refresh-banner-btn {
      border: none;
      border-radius: 6px;
      padding: 5px 12px;
      font-size: 12px;
    ...
    .agentdoc-refresh-revert {
      background: #007AFF;
      color: white;
    }
    .agentdoc-refresh-dismiss {
      font-size: 18px;
      padding: 2px 8px;
      color: #999;
    }
    .agentdoc-refresh-banner-btn:hover { opacity: 0.85; }
    .agentdoc-refresh-banner-btn:active { opacity: 0.7; }
  `;
