 ...
 *
 * Converts edits into agentdocSuggestion marks + AGENTDOC metadata
 ...
 * - Insertions get marked with agentdocSuggestion kind=insert
 * - Deletions get marked with agentdocSuggestion kind=delete instead of being removed
 * - Replacements become agentdocSuggestion kind=replace with content stored in metadata
 */
export function wrapTransactionForSuggestions(
  ...
  const suggestionType = state.schema.marks.agentdocSuggestion;

  if (!suggestionType) {
    console.warn('[suggestions] Missing agentdocSuggestion mark type');
    return tr;
  }
    ...
    if (stepJson.stepType === 'replace') {
      ...
      // CASE 2: Pure insertion (no deletion)
        ...
        if (candidate && whitespaceOnly) {
          ...
          // This keeps "Agentdoc is" as one suggestion instead of splitting at the space.
  ...
  return new Plugin<SuggestionState>({
    ...
    appendTransaction(_trs, oldState, newState) {
      ...
      if (wasEnabled !== isEnabled) {
        ...
        queueMicrotask(() => {
          (window as any).agentdoc?.bridge?.sendMessage('suggestionsChanged', { enabled: isEnabled });
        });
      }
      return null;
    },
