...
export function installCollabCursorStyles(): void {
  if (document.getElementById('agentdoc-collab-cursor-styles')) return;
  const style = document.createElement('style');
  style.id = 'agentdoc-collab-cursor-styles';
  style.textContent = `
    ...
    .ProseMirror > .ProseMirror-yjs-cursor.agentdoc-collab-cursor:first-child {
      margin-top: 16px;
    }

    .ProseMirror-yjs-cursor.agentdoc-collab-cursor {
      position: relative;
      pointer-events: none;
      display: inline-block;
      margin-left: -1px;
      margin-right: -1px;
      border-left: 2px solid var(--agentdoc-collab-cursor-color, #60a5fa);
      border-right: 0;
      word-break: normal;
    }

    .agentdoc-collab-cursor__label {
      position: absolute;
      left: -1px;
      top: -1.15em;
      max-width: 200px;
      ...
      border-left: 3px solid var(--agentdoc-collab-cursor-color, #60a5fa);
...
export function collabCursorBuilder(user: any): HTMLElement {
  ...
  cursorWidget.className = 'ProseMirror-yjs-cursor agentdoc-collab-cursor';
  cursorWidget.style.setProperty('--agentdoc-collab-cursor-color', color);

  const label = document.createElement('div');
  label.className = 'agentdoc-collab-cursor__label';
  if (shouldRenderAgentFace) {
    ...
    const icon = createAgentFaceElement({
      ...
      size: 14,
      title: `${name} icon`,
      wrapperClassName: 'agentdoc-collab-cursor__face',
      className: 'agentdoc-collab-cursor__face-svg',
    });
...
export function collabSelectionBuilder(user: any): DecorationAttrs {
  const color = normalizeColor(user?.color, '#60a5fa');
  return {
    class: 'ProseMirror-yjs-selection agentdoc-collab-selection',
    style: [
