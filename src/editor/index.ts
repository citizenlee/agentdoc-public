...
import { agentdocMarkPlugins } from './schema/agentdoc-marks';
...
import { agentdocMarkHandler } from '../formats/remark-agentdoc-marks';
import { remarkAgentdocMarksPlugin } from './schema/remark-agentdoc-marks-plugin';
...
// Global agentdoc interface for the web editor runtime
declare global {
  interface Window {
    agentdoc: AgentdocEditor;
  }
}
...
function stripAuthoredSpanTags(markdown: string): string {
  ...
  const authoredAttrRegex = /data-(?:proof|agentdoc)\s*=\s*(?:"authored"|'authored'|authored)/i;
...
function stripAgentdocSpanTags(markdown: string): string {
  const spanTagRegex = /<\/?span\b[^>]*>/gi;
  const agentdocAttrRegex = /data-(?:proof|agentdoc)\s*=/i;
  ...
  for (const match of markdown.matchAll(spanTagRegex)) {
    ...
    const isAgentdoc = agentdocAttrRegex.test(tag);
...
export interface AgentdocEditor {
  editor: Editor | null;
...
class AgentdocEditorImpl implements AgentdocEditor {
  ...
  constructor() {
    const agentdocConfig = (window as Window & {
      __AGENTDOC_CONFIG__?: { windowId?: string; documentId?: string };
    }).__AGENTDOC_CONFIG__ ?? {};

    if (!agentdocConfig.windowId) {
      agentdocConfig.windowId = `web-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
    }
    if (!agentdocConfig.documentId) {
      agentdocConfig.documentId = document.location.pathname || 'unknown';
    }

    (window as Window & { __AGENTDOC_CONFIG__?: { windowId?: string; documentId?: string } }).__AGENTDOC_CONFIG__ = agentdocConfig;

    const windowId = agentdocConfig.windowId;
    const documentId = agentdocConfig.documentId;

    initTelemetry({ windowId, documentId });

    initSessionManager({
      persistenceKey: `agentdoc-agent-sessions:${windowId ?? 'default'}:${documentId ?? 'unknown'}`,
  ...
  async init(): Promise<void> {
    ...
    this.editor = await editorBuilder
      // Register agentdoc mark schemas
      .use(agentdocMarkPlugins)
      // Register remark plugin for agentdoc marks parsing
      .use(remarkAgentdocMarksPlugin)
      // Register contexts
      .use(heatmapCtx)
      .use(agentCursorCtx)
      // Register plugins
      ...
      .config((ctx) => {
        // Note: remarkAgentdocMarks is now registered via .use(remarkAgentdocMarksPlugin)
    ...
    // Initialize agent integration for @agentdoc mentions
  ...
  private async initFromShare(options?: ShareRuntimeActivationOptions): Promise<void> {
    ...
    try {
      ...
      // Set title
      document.title = doc.title ? `${doc.title} - AgentDoc` : 'Shared Document - AgentDoc';
  ...
  private ensureShareBannerResponsiveCSS(): void {
    if (document.getElementById('agentdoc-share-banner-responsive-css')) return;
    const style = document.createElement('style');
    style.id = 'agentdoc-share-banner-responsive-css';
    style.textContent = `
      @keyframes agentdoc-agent-pulse {
      ...
      #share-banner .agentdoc-avatar-tooltip {
        position:absolute;
        top:calc(100% + 6px);
        left:50%;
      ...
      #share-banner .agentdoc-avatar-tooltip::before {
        content:'';
        position:absolute;
        bottom:100%;
        left:50%;
      ...
      #share-banner .agentdoc-avatar-wrap:hover .agentdoc-avatar-tooltip {
        opacity:1;
      }
      #share-banner .share-pill-agent-btn.menu-open .agentdoc-avatar-tooltip {
  ...
  private updateShareBannerPresenceDisplay(): void {
    ...
    for (let i = 0; i < Math.min(avatars.length, 5); i++) {
      ...
      wrap.className = 'agentdoc-avatar-wrap';
      ...
      tooltip.className = 'agentdoc-avatar-tooltip';
  ...
  private applyShareTitle(title: string | null | undefined): void {
    ...
    document.title = `${nextTitle} - AgentDoc`;
    this.updateShareBannerTitleDisplay();
  }
  ...
  private renderShareBannerContent(banner: HTMLElement, otherViewerCount: number): void {
    ...
    wordmark.href = 'https://agentdoc.up.railway.app';
  ...
  private getAgentInviteMessage(): string {
    ...
    if (!slug) {
      return [
        'Collaborate with me on this AgentDoc doc.',
        '',
        `Doc: ${shareUrl}`,
      ].join('\n');
    }
  ...
  private showShareWelcomeToastOnce(capabilities?: { canComment: boolean; canEdit: boolean } | null): void {
    ...
    try {
      const key = `agentdoc_share_welcome_${slug}`;
    ...
    toast.className = 'agentdoc-external-change-toast agentdoc-share-welcome-toast';
    toast.innerHTML = `
      <div class="agentdoc-toast-content">
        <span class="agentdoc-toast-message">${message}</span>
      </div>
    `;
  ...
  private createAgentMenuButton(
    ...
    const buildAgentFace = (
      ...
      face.style.animation = 'agentdoc-agent-pulse 2s ease-in-out infinite';
  ...
  /**
   * Initialize the agent integration for @agentdoc mentions.
  ...
  private initAgentIntegration(): void {
    ...
    this.editor.action((ctx) => {
      ...
      window.addEventListener('agentdoc:invoke-agent', handleAgentInvocation as EventListener);
  ...
  private invokeAgentOnSelection(
    ...
    // For now, create a comment with the request for @agentdoc to handle
    ...
    if (selectedText.trim()) {
      // Create a comment with @agentdoc mention to trigger the agent
      markComment(view, selectedText, actor, `@agentdoc ${prompt}`, context.range);
  ...
  private handleMarksChange(
    ...
    // Pass marks changes to agent integration for @agentdoc detection
  ...
  private normalizeMarkdownForRuntime(markdown: string): string {
    ...
    // Keep suggestion/comment agentdoc spans intact while removing authored wrappers.
  ...
  private normalizeMarkdownForCollab(markdown: string): string {
    ...
    return stripAgentdocSpanTags(this.normalizeMarkdownForRuntime(markdown));
  }
  ...
  loadDocument(content: string, options?: { allowShareContentMutation?: boolean }): void {
    ...
    this.editor.action((ctx) => {
      ...
      const applyLegacyMarks = (
        ...
        const suggestionMarkType = view.state.schema.marks.agentdocSuggestion;
        const commentMarkType = view.state.schema.marks.agentdocComment;
        const flaggedMarkType = view.state.schema.marks.agentdocFlagged;
        const approvedMarkType = view.state.schema.marks.agentdocApproved;
        const authoredMarkType = view.state.schema.marks.agentdocAuthored;
  ...
  rewriteDocument(
    ...
    this.editor.action((ctx) => {
      ...
      const incomingMarkdownForCompare = normalizeMarkdownForComparison(stripAgentdocSpanTags(canonicalizedContent));
      ...
      try {
        ...
        if (serialized) {
          currentMarkdownForCompare = normalizeMarkdownForComparison(stripAgentdocSpanTags(serialized));
        }
      } catch (error) {
  ...
  /**
   * Snapshot agentdocAuthored marks before loadDocument destroys them.
   */
  private snapshotProvenance(view: EditorView): Array<{ text: string; by: string }> {
    ...
    doc.descendants((node, _pos) => {
      ...
      const authoredMark = node.marks.find(m => m.type.name === 'agentdocAuthored');
      if (authoredMark) {
        snapshot.push({
          text: node.text || '',
  ...
  private restoreProvenance(
    ...
    doc.descendants((node, pos) => {
      ...
      const existingAuthored = node.marks.find(m => m.type.name === 'agentdocAuthored');
  ...
  private triggerRefreshFadeOut(): void {
    ...
    if (editor) {
      editor.classList.add('agentdoc-refreshing');
      editor.classList.remove('agentdoc-refreshed');
    }
  }
  ...
  private triggerRefreshFadeIn(): void {
    const editor = document.querySelector('.milkdown') as HTMLElement;
    if (editor) {
      editor.classList.remove('agentdoc-refreshing');
      editor.classList.add('agentdoc-refreshed');
      // Clean up class after animation
      setTimeout(() => editor.classList.remove('agentdoc-refreshed'), 400);
    }
  }
  ...
  private showExternalChangeToast(by: string, markCount: number): void {
    ...
    const toast = document.createElement('div');
    toast.className = 'agentdoc-external-change-toast';
    toast.innerHTML = `
      <div class="agentdoc-toast-content">
        <span class="agentdoc-toast-message">${displayName} updated ${markCount} section${markCount !== 1 ? 's' : ''}</span>
        <div class="agentdoc-toast-actions">
          <button class="agentdoc-toast-btn agentdoc-toast-review">Review</button>
          <button class="agentdoc-toast-btn agentdoc-toast-accept-all">Accept All</button>
          <button class="agentdoc-toast-btn agentdoc-toast-reject-all">Reject All</button>
        </div>
      </div>
    `;

    // Review button — scroll to first suggestion
    toast.querySelector('.agentdoc-toast-review')?.addEventListener('click', () => {
      this.navigateToFirstSuggestion();
      this.removeExternalChangeToast();
    });

    // Accept All
    toast.querySelector('.agentdoc-toast-accept-all')?.addEventListener('click', () => {
      this.markAcceptAll();
      this.removeExternalChangeToast();
    });

    // Reject All
    toast.querySelector('.agentdoc-toast-reject-all')?.addEventListener('click', () => {
      this.markRejectAll();
      this.removeExternalChangeToast();
    });
  ...
  private showRefreshBanner(by: string, stats?: ChangeStats): void {
    ...
    const banner = document.createElement('div');
    banner.className = 'agentdoc-refresh-banner';
    ...
    banner.innerHTML = `
      <div class="agentdoc-refresh-banner-info">
        <span class="agentdoc-refresh-banner-title">${displayName} rewrote this document</span>
        ${summary ? `<span class="agentdoc-refresh-banner-summary">${summary}</span>` : ''}
      </div>
      <div class="agentdoc-refresh-banner-actions">
        ${canRevert ? '<button class="agentdoc-refresh-banner-btn agentdoc-refresh-revert">Undo rewrite</button>' : ''}
        <button class="agentdoc-refresh-banner-btn agentdoc-refresh-dismiss">&times;</button>
      </div>
    `;

    if (canRevert) {
      banner.querySelector('.agentdoc-refresh-revert')?.addEventListener('click', () => {
        this.revertRefresh();
        this.removeRefreshBanner();
      });
    }

    banner.querySelector('.agentdoc-refresh-dismiss')?.addEventListener('click', () => {
      this.removeRefreshBanner();
    });
  ...
  setApiKey(apiKey: string): void {
    ...
    console.log('[agentdoc] API key configured');
  ...
  async runSkillReview(skillId: string, scope: 'selection' | 'document'): Promise<void> {
    if (!this.editor) {
      console.error('[agentdoc] Cannot run skill review: editor not initialized');
    ...
    const actionPromise = this.editor.action(async (ctx) => {
      ...
      try {
        await reviewPromise;
        runSucceeded = true;
        console.log(`[agentdoc] Skill review completed: ${skill.name}`);
      } catch (error) {
        runError = error instanceof Error ? error.name : 'unknown_error';
        console.error(`[agentdoc] Skill review failed: ${skill.name}`, error);
      }
    });
  ...
  async debugPlanOnly(
    ...
    if (!this.editor) {
      console.error('[agentdoc] Cannot run plan-only debug: editor not initialized');
      return null;
    }
    ...
    const skill = registry.getSkill(skillId);
    if (!skill) {
      console.error(`[agentdoc] Unknown skill for plan-only debug: ${skillId}`);
      return null;
    }

    if (this.reviewInFlight) {
      console.warn(`[agentdoc] Review already running; ignoring plan-only request for ${skillId}`);
      return null;
    }
    ...
    try {
      ...
      console.log('[agentdoc] Plan-only debug result:', result);
      return result;
    } finally {
      if (this.reviewInFlight === actionPromise) {
        this.reviewInFlight = null;
  ...
  async debugRunSingleFocusArea(
    ...
    if (!this.editor) {
      console.error('[agentdoc] Cannot run single focus-area debug: editor not initialized');
      return null;
    }
    ...
    if (this.reviewInFlight) {
      console.warn(`[agentdoc] Review already running; ignoring single focus-area request for ${skillId}`);
      return null;
    }
    ...
    try {
      const result = await actionPromise;
      console.log('[agentdoc] Single focus-area debug result:', result);
      return result;
    } finally {
      if (this.reviewInFlight === actionPromise) {
        this.reviewInFlight = null;
  ...
  async debugRunOrchestrated(
    ...
    if (!this.editor) {
      console.error('[agentdoc] Cannot run orchestrated debug: editor not initialized');
      return null;
    }
    ...
    const skill = registry.getSkill(skillId);
    if (!skill) {
      console.error(`[agentdoc] Unknown skill for orchestrated debug: ${skillId}`);
      return null;
    }
    ...
    if (options?.cancelActive) {
      try {
        ...
        console.warn('[agentdoc] Failed to cancel active review before orchestrated debug:', error);
      } finally {
        this.reviewInFlight = null;
      }
    }
    ...
    if (this.reviewInFlight) {
      console.warn(`[agentdoc] Review already running; ignoring orchestrated debug request for ${skillId}`);
      return null;
    }
    ...
    try {
      const result = await actionPromise;
      console.log('[agentdoc] Orchestrated debug result:', result);
      return result;
    } finally {
      if (this.reviewInFlight === actionPromise) {
        this.reviewInFlight = null;
  ...
  debugMapTextOffsets(from: number, to: number): { range: MarkRange | null; quote: string | null } | null {
    if (!this.editor) {
      console.error('[agentdoc] Cannot map text offsets: editor not initialized');
      return null;
    }
  ...
  debugDescribeTextblocks(range: { from: number; to: number }): unknown {
    if (!this.editor) {
      console.error('[agentdoc] Cannot describe textblocks: editor not initialized');
      return null;
    }
  ...
  debugTextForRange(range: MarkRange): { range: MarkRange; text: string } | null {
    if (!this.editor) {
      console.error('[agentdoc] Cannot read text for range: editor not initialized');
      return null;
    }
  ...
  debugGetParagraphCandidate(minWords: number = 8): { range: MarkRange; quote: string; line: number } | null {
    if (!this.editor) {
      console.error('[agentdoc] Cannot get paragraph candidate: editor not initialized');
      return null;
    }
  ...
  debugFindBlockByText(text: string): { range: MarkRange; nodeType: string; line: number } | null {
    if (!this.editor) {
      console.error('[agentdoc] Cannot find block by text: editor not initialized');
      return null;
    }
  ...
  debugInspectMarksForText(target: string): {
    ...
    if (!this.editor) {
      console.error('[agentdoc] Cannot inspect marks: editor not initialized');
      return null;
    }
  ...
  debugResolveRangeWithValidation(quote: string, range?: MarkRange): unknown {
    if (!this.editor) {
      console.error('[agentdoc] Cannot resolve range: editor not initialized');
      return null;
    }
  ...
  debugAnalyzeReplace(quote: string, content: string, range?: MarkRange): unknown {
    if (!this.editor) {
      console.error('[agentdoc] Cannot analyze replace: editor not initialized');
      return null;
    }
  ...
  debugQuoteSpansMultipleTableCells(quote: string, range?: MarkRange): boolean {
    if (!this.editor) {
      console.error('[agentdoc] Cannot check table-cell boundary: editor not initialized');
      return false;
    }
  ...
  async cancelReview(): Promise<void> {
    ...
    try {
      ...
      console.error('[agentdoc] Failed to cancel review:', error);
    } finally {
      this.reviewInFlight = null;
  ...
  async stopAllReviews(): Promise<{ cancelledSessions: number; unlocked: boolean; lockCount: number }> {
    ...
    try {
      ...
      console.error('[agentdoc] Failed to cancel active review executor:', error);
    }
    ...
    try {
      ...
      console.error('[agentdoc] Failed to cancel agent sessions:', error);
    } finally {
    ...
    if (lockState.locked) {
      console.warn(
        `[agentdoc] stopAllReviews: review lock still held after ${unlockAttempts} unlock attempts (lockCount=${lockState.lockCount}).`
      );
    } else {
      console.log(
        `[agentdoc] stopAllReviews: cancelledSessions=${cancelledSessions} unlockAttempts=${unlockAttempts}.`
      );
    }
...
// Initialize global agentdoc instance
window.agentdoc = new AgentdocEditorImpl();
const externalAgentHooks = (window as any).__agentdocExternalAgentHooks;
if (externalAgentHooks && typeof externalAgentHooks === 'object') {
  Object.assign(window.agentdoc, externalAgentHooks);
}

// Minimal browser-agent hook. Only exposed on share pages (/d/:slug).
if (window.location?.pathname?.startsWith('/d/')) {
  (window as any).__AGENTDOC_EDITOR__ = {
    getMarkdown: () => (window.agentdoc.getMarkdownSnapshot()?.content ?? ''),
    insertMarkdown: (markdown: string) => window.agentdoc.insertAtCursor(markdown, 'ai:browser-agent'),
    replaceSelection: (markdown: string) => window.agentdoc.replaceSelection(markdown, 'ai:browser-agent'),
    focus: () => {
      try {
...
(window as any).sendAgentPrompt = (prompt: string) => {
  ...
  try {
    const impl = window.agentdoc as AgentdocEditorImpl;
    if (impl?.editor) {
...
(window as any).validateMarkAnchors = () => window.agentdoc.validateMarkAnchors();
(window as any).debugPlanOnly = (skillId: string, options?: unknown) => window.agentdoc.debugPlanOnly(skillId, options);
(window as any).debugRunSingleFocusArea = (skillId: string, options?: unknown) =>
  window.agentdoc.debugRunSingleFocusArea(skillId, options);
(window as any).debugGetCachedPlan = (skillId: string) => window.agentdoc.debugGetCachedPlan(skillId);
(window as any).debugClearPlanCache = (skillId?: string) => window.agentdoc.debugClearPlanCache(skillId);
(window as any).debugRunOrchestrated = (skillId: string, options?: unknown) =>
  window.agentdoc.debugRunOrchestrated(skillId, options);
(window as any).debugMapTextOffsets = (from: number, to: number) => window.agentdoc.debugMapTextOffsets(from, to);
(window as any).debugDescribeTextblocks = (from: number, to: number) =>
  window.agentdoc.debugDescribeTextblocks({ from, to });
(window as any).debugTextForRange = (from: number, to: number) =>
  window.agentdoc.debugTextForRange({ from, to });
(window as any).debugGetParagraphCandidate = (minWords?: number) =>
  window.agentdoc.debugGetParagraphCandidate(minWords);
(window as any).debugFindBlockByText = (text: string) => window.agentdoc.debugFindBlockByText(text);
(window as any).debugInspectMarksForText = (text: string) => window.agentdoc.debugInspectMarksForText(text);
(window as any).debugResolveRangeWithValidation = (quote: string, range?: MarkRange) =>
  window.agentdoc.debugResolveRangeWithValidation(quote, range);
(window as any).debugAnalyzeReplace = (quote: string, content: string, range?: MarkRange) =>
  window.agentdoc.debugAnalyzeReplace(quote, content, range);

// Comment navigation shortcuts for devtools/testing hooks
(window as any).navigateNextComment = () => window.agentdoc.navigateToNextComment();
(window as any).navigatePrevComment = () => window.agentdoc.navigateToPrevComment();
(window as any).resolveActiveComment = () => window.agentdoc.resolveActiveComment();

// Auto-initialize when DOM is ready
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', () => {
    console.log('[INIT] DOMContentLoaded - calling init()');
    window.agentdoc.init();
  });
} else {
...
export default window.agentdoc;
