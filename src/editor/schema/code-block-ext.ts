 ...
 *
 * Extends the default code_block schema to allow agentdoc marks on code blocks.
...
// Types for encoded agentdoc marks
interface EncodedMark {
  type: string;  // 'agentdocComment', 'agentdocAuthored', etc.
  from: number;
  to: number;
...
/**
 * Extract agentdoc marks from a code block node
 */
function extractAgentdocMarks(node: ProseMirrorNode): EncodedMark[] {
  ...
  node.forEach((child, childOffset) => {
    if (child.isText && child.marks.length > 0) {
      for (const mark of child.marks) {
        // Only encode agentdoc marks
        if (mark.type.name.startsWith('agentdoc') || mark.type.name.startsWith('proof')) {
          marks.push({
            type: mark.type.name,
            from: offset,
...
function encodeMarksForMeta(marks: EncodedMark[]): string {
  ...
  return 'agentdoc:' + base64EncodeUtf8(json);
}
...
function decodeMarksFromMeta(meta: string | undefined): EncodedMark[] {
  ...
  // Look for agentdoc: prefix first, then fall back to proof: prefix
  const agentdocPrefix = 'agentdoc:';
  const legacyPrefix = 'proof:';
  let prefixIndex = meta.indexOf(agentdocPrefix);
  let prefixLength = agentdocPrefix.length;
  if (prefixIndex === -1) {
    prefixIndex = meta.indexOf(legacyPrefix);
    prefixLength = legacyPrefix.length;
  }
  if (prefixIndex === -1) return [];

  // Extract the base64 part (everything after prefix until space or end)
  const start = prefixIndex + prefixLength;
  ...
  try {
    ...
    console.warn('[code-block-ext] Failed to decode agentdoc marks from meta:', e);
    return [];
  }
}
...
/**
 * Remove agentdoc metadata from meta string (for display purposes)
 */
function stripAgentdocFromMeta(meta: string | undefined): string {
  if (!meta) return '';
  return meta.replace(/\s*(?:agentdoc|proof):[A-Za-z0-9+/=]+\s*/g, '').trim();
}
...
export const codeBlockSchemaExt = $nodeSchema('code_block', (ctx) => {
  return {
    content: 'text*',
    group: 'block',
    marks: 'agentdocAuthored agentdocSuggestion agentdocComment agentdocFlagged agentdocApproved',
    defining: true,
    code: true,
    attrs: {
      language: {
    ...
    parseMarkdown: {
      ...
      runner: (state, node, type) => {
        ...
        // Decode any agentdoc marks from meta
        const agentdocMarks = decodeMarksFromMeta(meta);

        state.openNode(type, { language });

        if (value) {
          if (agentdocMarks.length === 0) {
            ...
            for (const mark of agentdocMarks) {
    ...
    toMarkdown: {
      ...
      runner: (state, node) => {
        // Extract agentdoc marks from the code block
        const agentdocMarks = extractAgentdocMarks(node);
        const encodedMarks = encodeMarksForMeta(agentdocMarks);

        // Build meta: language first, then agentdoc marks if any
