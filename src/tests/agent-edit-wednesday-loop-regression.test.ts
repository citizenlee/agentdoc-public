...
const WEDNESDAY_TASK_CLUSTER = [
  ...
  '',
  '**Agentdoc Review** - one consolidated session with R2C2',
].join('\n');
...
const DOC_PREFIX = [
  ...
  '## Focus (Feb 21 -- Mar 7)',
  '',
  'Make Agentdoc the best collaborative markdown editor for AI and humans.',
  '',
  '***',
  '',
].join('\n');
...
async function run(): Promise<void> {
  const dbName = `agentdoc-agent-edit-wednesday-loop-${Date.now()}.db`;
