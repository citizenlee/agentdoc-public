...
test('runtime config commentUiDefaultMode is used when query/storage are absent', () => {
  withMockWindow({
    ...
    __AGENTDOC_CONFIG__: { commentUiDefaultMode: 'legacy' },
    innerWidth: 1200,
    matchMedia: (_query: string) => ({ matches: false }),
  }, () => {
...
test('source includes selection caching + pointer/touch handlers + arrow trigger plugin', () => {
  ...
  assert(popover.includes('function getAgentdocEditorApi(): Window[\'agentdoc\'] | null {'), 'Popover should be able to reach the share-aware editor API when available');
  assert(popover.includes('agentdoc.markAccept(mark.id);'), 'Suggestion apply action should use the share-aware editor API');
  assert(popover.includes('agentdoc.markReject(mark.id);'), 'Suggestion reject action should use the share-aware editor API');
  assert(popover.includes('agentdoc.markResolve(mark.id);'), 'Thread resolve action should use the share-aware editor API');
  assert(popover.includes('agentdoc.markUnresolve(mark.id);'), 'Thread reopen action should use the share-aware editor API');
  assert(popover.includes("? agentdoc.markReply(mark.id, getCurrentActor(), text)"), 'Thread reply action should use the share-aware editor API');
  ...
  assert(popover.includes('agentdoc.markDeleteThread(mark.id);'), 'Thread delete action should use the share-aware editor API');
  ...
  assert(commentUiMode.includes('__AGENTDOC_CONFIG__?.commentUiDefaultMode'), 'Comment UI mode should honor the server-provided runtime default');
  ...
  assert(indexSource.includes('agentdoc-share-welcome-toast'), 'Expected share welcome toast to use mobile-safe class');
