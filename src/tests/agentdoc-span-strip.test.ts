import {
  ...
  stripAgentdocSpanTags,
} from '../../server/agentdoc-span-strip.ts';
...
function run(): void {
  ...
  const stripped = stripAgentdocSpanTags(markdown);
  assertEqual(
    stripped,
  ...
  assertEqual(mapped, stripped, 'Expected stripped index map to preserve the same output as stripAgentdocSpanTags');
  ...
  const allStripped = stripAllAgentdocSpanTags(markdown);
  assertEqual(allStripped, 'Hello draft and commented text.', 'Expected all Agentdoc spans to be stripped');
  ...
  const repairedBase = stripAllAgentdocSpanTagsWithReplacements(staleSuggestionMarkdown, {
    s1: 'restored quote text',
  });
  assertEqual(
    repairedBase,
    'Before restored quote text after.',
    'Expected replacement-aware stripping to rebuild the agentdoc-span-free base text',
  );
  ...
  const splitBase = stripAllAgentdocSpanTagsWithReplacements(splitSuggestionMarkdown, {
  ...
  console.log('✓ agentdoc span stripping preserves non-authored marks');
}
