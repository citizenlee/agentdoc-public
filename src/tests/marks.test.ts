...
import { agentdocMarkHandler, remarkAgentdocMarks } from '../formats/remark-agentdoc-marks.js';
...
test('applyRemoteMarks does not resurrect locally rejected mark ids', () => {
  ...
  const suggestionMark = marksSchema.marks.agentdocSuggestion.create({
    id: markId,
    kind: 'replace',
    by: 'ai:test',
  });
...
test('applyRemoteMarks reanchors authored marks from relative anchors when quote is missing', () => {
  ...
  seededState = seededState.apply(
    seededState.tr.addMark(
      betaRange!.from,
      betaRange!.to,
      authoredSchema.marks.agentdocAuthored.create({ by: 'human:michael' }),
    ),
  );
...
test('applyRemoteMarks does not throttle hydration across different docs with same mark id', () => {
  ...
  secondState.doc.nodesBetween(0, secondState.doc.content.size, (node) => {
    ...
    if (node.marks.some((mark) => mark.type.name === 'agentdocComment' && mark.attrs?.id === markId)) {
...
test('applyRemoteMarks preserves existing comment payload when incoming metadata is partial', () => {
  ...
  const commentMark = marksSchema.marks.agentdocComment.create({ id: commentId, by: 'human:dan' });
  const doc = marksSchema.node('doc', null, [
    marksSchema.node('paragraph', null, [
      marksSchema.text('Hello '),
...
test('comment anchors without metadata are not surfaced or flushed as empty comments', () => {
  const commentMark = marksSchema.marks.agentdocComment.create({ id: 'c1', by: 'human:dan' });
  const doc = marksSchema.node('doc', null, [
    marksSchema.node('paragraph', null, [
      marksSchema.text('Hello '),
      marksSchema.text('world', [commentMark]),
...
test('comment anchors with empty text metadata are treated as invalid (not surfaced or flushed)', () => {
  const commentMark = marksSchema.marks.agentdocComment.create({ id: 'c2', by: 'human:dan' });
  const doc = marksSchema.node('doc', null, [
    marksSchema.node('paragraph', null, [
      marksSchema.text('Hello '),
      marksSchema.text('world', [commentMark]),
...
test('resolveMarks returns multiple ranges for split marks', () => {
  const markType = marksSchema.marks.agentdocSuggestion;
...
test('embedMarks adds marks block to markdown', () => {
  ...
  assert(result.includes('<!-- AGENTDOC'), 'Should have marks block');
...
test('extractMarks extracts metadata from markdown', () => {
  ...
  assert(!content.includes('<!-- AGENTDOC'), 'Content should not have marks block');
});
...
test('agentdocMarkHandler renders inline formatting as markdown', () => {
  const node = {
    type: 'agentdocMark',
    markKind: 'comment',
  ...
  const md = agentdocMarkHandler(node as any);
  assert(md.includes('data-agentdoc="comment"'), 'Should include agentdoc attribute');
  assert(md.includes('**Hello**'), 'Should render strong as markdown');
  assert(!md.includes('<strong>'), 'Should not emit HTML strong tags');
});

test('agentdocMarkHandler renders suggestion spans with kind', () => {
  const node = {
    type: 'agentdocMark',
    markKind: 'suggestion',
  ...
  const html = agentdocMarkHandler(node as any);
  assert(html.includes('data-agentdoc="suggestion"'), 'Should include suggestion agentdoc attribute');
...
test('remarkAgentdocMarks converts split agentdoc spans into nodes', () => {
  const tree = {
    type: 'root',
    children: [
      {
  ...
  remarkAgentdocMarks()(tree as any);
  const paragraph = (tree.children as any[])[0];
  const children = paragraph.children as any[];
  assertEqual(children.length, 1, 'Expected a single agentdocMark node');
  const mark = children[0];
  assertEqual(mark.type, 'agentdocMark');
  assertEqual(mark.markKind, 'comment');
  assertEqual(mark.attrs?.id, 'm1');
  assertEqual(mark.attrs?.by, 'human:dan');
  assertEqual(mark.children?.[0]?.value, 'Hello');
});

test('remarkAgentdocMarks preserves suggestion kind attrs', () => {
  const tree = {
    type: 'root',
    children: [
      {
  ...
  const mark = children[0];
  assertEqual(mark.type, 'agentdocMark');
  assertEqual(mark.markKind, 'suggestion');
  assertEqual(mark.attrs?.id, 'm3');
  assertEqual(mark.attrs?.by, 'ai:test');
  assertEqual(mark.attrs?.kind, 'insert');
});

test('remarkAgentdocMarks converts legacy <code> html nodes to inlineCode (split path)', () => {
...
test('parseAgentdocHtml handles <code> in inline agentdoc spans', () => {
...
test('agentdocMarkHandler renders authored spans with markdown formatting', () => {
  const node = {
    type: 'agentdocMark',
    markKind: 'authored',
    attrs: { by: 'human:dan' },
  ...
  const md = agentdocMarkHandler(node as any);
  assert(md.includes('data-agentdoc="authored"'), 'Should include authored agentdoc attribute');
...
test('extractEmbeddedProvenance preserves authored agentdoc spans', () => {
...
test('extractEmbeddedProvenance preserves non-authored agentdoc spans', () => {
...
test('accept parses markdown replacements into inline marks when parser is provided', () => {
  ...
  const suggestionMark = schema.marks.agentdocSuggestion.create({
    id: markId,
    kind: 'replace',
    by: 'ai:test',
  });
  ...
  state.doc.descendants((node) => {
    ...
    for (const mark of node.marks) {
      ...
      if (mark.type.name === 'agentdocSuggestion') hasSuggestion = true;
    }
    return true;
  });
...
test('accept preserves heading style for non-structural full-block replacements', () => {
  ...
  const replacement = 'Feature 1: Always-On Agentdoc Agent (Double-Click Sidebar)';
  const metadata = {
    [markId]: {
      kind: 'replace' as const,
      by: 'ai:test',
...
test('acceptAll applies multiple replacements without duplicating characters', () => {
  ...
  const firstMark = schema.marks.agentdocSuggestion.create({ id: firstId, kind: 'replace', by: 'ai:test' });
  const secondMark = schema.marks.agentdocSuggestion.create({ id: secondId, kind: 'replace', by: 'ai:test' });
  const thirdMark = schema.marks.agentdocSuggestion.create({ id: thirdId, kind: 'replace', by: 'ai:test' });
