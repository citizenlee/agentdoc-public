...
async function run(): Promise<void> {
  const dbName = `agentdoc-collab-put-propagates-${Date.now()}-${randomUUID()}.db`;
  ...
  try {
    ...
    const pollutedMarkdown = '# Content C\n\nBefore <span class="ProseMirror-yjs-cursor agentdoc-collab-cursor">\u2060cursor\u2060</span> After.';
    ...
    assert(!sanitizedMarkdown.includes('agentdoc-collab-cursor'), 'Expected collab markdown to strip agentdoc collab cursor span');
