...
const CLIENT_HEADERS = {
  'X-Agentdoc-Client-Version': '0.31.0',
...
async function withEphemeralApiServer(run: (baseUrl: string) => Promise<void>): Promise<void> {
  const dbName = `agentdoc-share-routes-${Date.now()}-${Math.random().toString(36).slice(2)}.db`;
...
async function runServerHookTests(): Promise<void> {
  ...
  await test('D1: /agent-setup returns streamlined setup guide content', async () => {
    ...
    assertIncludes(body, 'Agentdoc — Agent Setup', 'agent-setup should include heading text');
  ...
  await test('D1: /codex-agent-setup redirects to the unified setup guide', async () => {
    ...
    const body = response.body || '';
    assertIncludes(body, 'Agentdoc — Agent Setup', 'codex-agent-setup should resolve to unified setup text');
    assertIncludes(body, 'agentdoc.SKILL.md', 'codex-agent-setup should include the unified skill install URL');
  });
  ...
  await test('D1: /proof.SKILL.md returns the unified skill', async () => {
    ...
    const body = response.body || '';
    assertIncludes(body, 'name: proof', 'skill should include agentdoc frontmatter');
    assertIncludes(body, '# Agentdoc', 'skill should include the expected heading');
  });
  ...
  await test('D1: /proof-agent-ops.SKILL.md remains a compatibility alias', async () => {
    ...
    assertIncludes(body, '# Agentdoc', 'compat route should serve the unified skill content');
  });
  ...
  await test('D1: /agent-setup includes unified fail-fast skill install commands', async () => {
    ...
    assertIncludes(
      ...
      'agent-setup should reference the installed codex agentdoc skill path',
    );
  });
  ...
  await test('D1: landing page Codex copy prompt includes unified skill install command', async () => {
    ...
    assertIncludes(
      body,
      '<title>Agentdoc — A collaborative editor for humans and AI</title>',
    ...
    assertIncludes(
      ...
      'landing page should include a configurable local Agentdoc SDK base URL',
    );
    assertIncludes(
      body,
      '.hero-text + .btn-primary',
  ...
  await withEphemeralApiServer(async (baseUrl) => {
    ...
    await test('D2: POST /api/documents in warn mode returns deprecation headers + metadata', async () => {
      ...
      try {
        ...
        assert((response.headers.get('x-agentdoc-legacy-create') || '') === 'warn', 'Expected x-agentdoc-legacy-create=warn');
        const payload = await response.json();
    ...
    await test('D2: POST /api/documents in disabled mode returns LEGACY_CREATE_DISABLED', async () => {
      ...
      try {
        ...
        assert((response.headers.get('x-agentdoc-legacy-create') || '') === 'disabled', 'Expected x-agentdoc-legacy-create=disabled');
        const payload = await response.json();
    ...
    await test('D2: /d/:slug Slack unfurl bots get HTML metadata without explicit Accept', async () => {
      ...
      assertIncludes(body, '<title>Validation test | Agentdoc</title>', 'Expected document-specific title in Slack unfurl HTML');
    });
    ...
    await test('D2: /d/:slug HTML response includes agent discovery meta tags + noscript fallback', async () => {
      ...
      assertIncludes(body, '<title>Validation test | Agentdoc</title>', 'Expected dynamic document title in HTML head');
      ...
      assert(
        !body.includes('<title>Agentdoc Editor</title>'),
    ...
    await test('D2: unavailable shares emit generic social metadata without doc excerpt', async () => {
      ...
      assertIncludes(body, 'content="The shared Agentdoc document is temporarily unavailable"', 'Expected generic unavailable metadata');
    ...
    await test('D2: long OG titles shrink to preserve description space', async () => {
      ...
      const longLayout = resolveOgTextLayout('How We Want the Agentdoc SDK Bundle to Feel When Someone First Realizes It Can Actually Do Computer Errands Across the Entire Writing Stack Including Review, Rewrite, Comments, Provenance, and Collaboration');
    ...
    await test('D2: /d/:slug HTML response injects runtime comment UI default mode when configured', async () => {
      ...
      try {
        ...
        assertIncludes(
          response.body || '',
          'window.__AGENTDOC_CONFIG__.commentUiDefaultMode = "legacy";',
    ...
    await test('D2: collab-session rewrites localhost ws URL for forwarded public hosts', async () => {
      ...
      try {
        const response = await get(baseUrl, `/api/documents/${slug}/collab-session`, {
          ...
          'x-forwarded-host': 'www.agentdoc.com',
        });
        assert(response.status === 200, `Expected status 200, got ${response.status}`);
        const payload = await response.json();
        if (payload?.session) {
          const collabWsUrl = String(payload?.session?.collabWsUrl ?? '');
          assert(collabWsUrl.startsWith('wss://www.agentdoc.com'), `Expected forwarded collab ws URL, got ${collabWsUrl}`);
    ...
    await test('D2: POST /share/markdown defaults to anonymous create in Agentdoc SDK', async () => {
      const previousMode = process.env.PROOF_SHARE_MARKDOWN_AUTH_MODE;
