...
function run(): void {
  ...
  assert(resolveAgentFamily({ avatar: 'https://cdn.example.com/agentdoc-agent-yellow.svg' }) === 'yellow', 'Expected avatar URL to resolve the yellow icon');

  assert(isAgentIdentity({ name: 'AgentDoc Agent' }) === true, 'Expected AgentDoc Agent to be treated as an agent');
