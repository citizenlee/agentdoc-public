...
const FIXTURE = [
  ...
  '**📅 Planning**',
  '',
  '* [ ] Figure out calendar and scheduling for Agentdoc / Sheriff / Claw launches',
  '',
  `* [ ] ${ANCHOR}`,
  '',
...
async function run(): Promise<void> {
  const dbName = `agentdoc-agent-edit-repeated-anchor-${Date.now()}-${Math.random().toString(36).slice(2)}.db`;
