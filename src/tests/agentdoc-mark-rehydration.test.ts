...
async function run(): Promise<void> {
  const dbName = `agentdoc-mark-rehydration-${Date.now()}-${Math.random().toString(36).slice(2)}.db`;
  ...
  const { rehydrateAgentdocMarksMarkdown } = await import('../../server/agentdoc-mark-rehydration.ts');
  const { repairAgentdocMarksForSlug } = await import('../../server/agentdoc-mark-repair.ts');
  ...
  try {
    ...
    const fullQuote = 'You can try it yourself right now. A copy of this post is available on Agentdoc. Use the share link there to have Claude, ChatGPT, your claw, or any other agent add their comments.';
    const truncatedQuote = fullQuote.slice(0, 100);
    const replacement = 'Agentdoc lets you share a doc with an agent and review its edits inline.';
    ...
    const rejectedVisibleText = stripAllAgentdocSpanTags(rejectedDoc?.markdown ?? '');
    ...
    assertEqual(
      stripAgentdocSpanTags(longDoc?.markdown ?? ''),
    ...
    assert(
      stripAgentdocSpanTags(inertCommentDoc?.markdown ?? '').includes('Hi there'),
    ...
    const nestedRepair = await rehydrateAgentdocMarksMarkdown(nestedMarkdown, nestedMarks);
    assert(nestedRepair.ok, `Expected nested repair to succeed, got ${nestedRepair.ok ? 'ok' : nestedRepair.error}`);
    const nestedVisibleText = stripAllAgentdocSpanTags(nestedRepair.markdown);
    ...
    const repairReport = await repairAgentdocMarksForSlug(nestedSlug, { write: true });
    ...
    assertEqual(
      stripAllAgentdocSpanTags(splitAcceptedDoc?.markdown ?? '').trim(),
      splitBase,
    ...
    assertEqual(
      stripAllAgentdocSpanTags(splitRejectedDoc?.markdown ?? '').trim(),
      splitBase,
    ...
    const splitRepair = await rehydrateAgentdocMarksMarkdown(splitRepairFixture.markdown, splitRepairFixture.marks);
    assert(splitRepair.ok, `Expected split repair to succeed, got ${splitRepair.ok ? 'ok' : splitRepair.error}`);
    assertEqual(
      stripAllAgentdocSpanTags(splitRepair.markdown).trim(),
      splitBase,
    ...
    const splitRepairReport = await repairAgentdocMarksForSlug(splitRepairSlug, { write: true });
    ...
    assertEqual(
      stripAllAgentdocSpanTags(repairedSplitDoc?.markdown ?? '').trim(),
      splitBase,
    ...
    assert(
      stripAllAgentdocSpanTags(staleAuthoredDoc?.markdown ?? '').includes('Before Hi After'),
    ...
    assert(
      (missingAuthoredDoc?.markdown ?? '').includes('data-agentdoc="authored"') || (missingAuthoredDoc?.markdown ?? '').includes('data-proof="authored"'),
    ...
    assertEqual(
      stripAllAgentdocSpanTags(authoredAcceptedDoc?.markdown ?? '').trim(),
    ...
    assertEqual(
      (authoredAcceptedDoc?.markdown.match(/data-(?:proof|agentdoc)="authored"[^>]*data-by="human:dan"/g) ?? []).length,
      2,
    ...
    assertEqual(
      stripAllAgentdocSpanTags(authoredRejectedDoc?.markdown ?? '').trim(),
      authoredBase,
      'Expected authored preservation reject to keep the original text without dropping authored spans',
    );
    assertEqual(
      (authoredRejectedDoc?.markdown.match(/data-(?:proof|agentdoc)="authored"[^>]*data-by="human:dan"/g) ?? []).length,
      2,
    ...
    console.log('✓ agentdoc mark rehydration repairs legacy accept/reject/add-accepted and nested repair flows');
  } finally {
    try {
      unlinkSync(dbPath);
    } catch {
