/**
 * AgentDoc Document Tools
 *
 * Tools for interacting with the AgentDoc document editor.
...
export function setEditorView(view: EditorView): void {
  editorView = view;
  console.log('[AgentDoc] Editor view set');
}
...
/**
 * Get the Agentdoc document tools
 */
export function getAgentdocTools(context: DocumentContext): AgentTool[] {
  return [
    // Read document
    {
      name: 'read_document',
    ...
    // Show conflict dialog
    {
      ...
      handler: async (args) => {
        ...
        window.dispatchEvent(new CustomEvent('agentdoc:conflict-dialog', { detail: { message } }));
        console.warn('[AgentDoc] Conflict dialog requested:', message);
...
async function performInsert(
  ...
  console.log('[AgentDoc] Inserted text at position', pos);
  return {
    success: true,
...
async function performReplace(
  ...
  console.log('[AgentDoc] Replaced text at range', range);
  return {
    success: true,
    selector,
    position: range,
...
async function performDelete(selector: string, context: DocumentContext): Promise<unknown> {
  ...
  console.log('[AgentDoc] Deleted text at range', range);
  return {
    success: true,
    selector,
    position: range,
...
async function createSuggestion(
  ...
  console.log('[AgentDoc] createSuggestion called:', { type, text: text?.substring(0, 50), selector });

  if (!editorView) {
    console.error('[AgentDoc] Editor view is null!');
    return { success: false, error: 'Editor not available' };
  }
  ...
  switch (type) {
    case 'insert':
      ...
      console.log('[AgentDoc] Created insert suggestion', mark?.id);
      break;

    case 'delete':
      mark = suggestDelete(editorView, quote, AGENT_ACTOR, range);
      console.log('[AgentDoc] Created delete suggestion', mark?.id);
      break;
    ...
    case 'replace':
      ...
      console.log('[AgentDoc] Created replace suggestion', mark?.id);
      break;
  }
...
async function addComment(
  ...
  console.log('[AgentDoc] Added comment', mark.id);
  return {
    success: true,
    commentId: mark.id,
    text,
...
async function replyToComment(commentId: string, text: string): Promise<unknown> {
  ...
  console.log('[AgentDoc] Replied to comment', commentId, 'with', mark.id);
  return {
    success: true,
    commentId,
    replyId: mark.id,
...
async function resolveComment(commentId: string): Promise<unknown> {
  ...
  console.log('[AgentDoc] Resolved comment', commentId);
