...
import {
  buildAgentdocSpanReplacementMap,
  stripAllAgentdocSpanTagsWithReplacements,
} from './agentdoc-span-strip.js';
import {
  extractAuthoredMarksFromDoc,
  synchronizeAuthoredMarks,
} from './agentdoc-authored-mark-sync.js';
...
export async function deriveProjectionFromCanonicalDoc(
  ...
  const strippedMarkdown = stripEphemeralCollabSpans(markdown);

  // If the Yjs Y.Text('markdown') contains table pipes but the fragment-derived
  // markdown does not, the serializer failed to round-trip GFM tables.  Fall back
  // to the canonical Y.Text markdown to avoid silently dropping table content.
  const canonicalMarkdown = stripEphemeralCollabSpans(ydoc.getText('markdown').toString());
  if (canonicalMarkdown.includes('|') && !strippedMarkdown.includes('|')) {
    console.warn('[canonical] deriveProjectionFromCanonicalDoc: fragment-derived markdown lost table pipes; using Y.Text markdown');
    return {
...
export async function mutateCanonicalDocument(args: CanonicalMutationArgs): Promise<CanonicalMutationResult> {
  ...
  try {
    ...
    if (args.guardPathologicalGrowth !== false) {
      const guardBaselineMarkdown = stripAllAgentdocSpanTagsWithReplacements(
        authoritativeMarkdown,
        buildAgentdocSpanReplacementMap(authoritativeMarks),
      );
      const guardCandidateMarkdown = stripAllAgentdocSpanTagsWithReplacements(
        sanitizedMarkdown,
        buildAgentdocSpanReplacementMap(effectiveNextMarks),
      );
    ...
    try {
      saveDocumentVersion(args.slug, sanitizedMarkdown, updated.title, updated.revision, args.actor ?? null);
    } catch (e) {
      console.error('[canonical-document] Failed to save version snapshot:', e);
    }
...
export async function executeCanonicalRewrite(
  ...
  // Full content rewrite: clear all marks (comments, suggestions, authored).
  // Save the pre-rewrite state (with comments) to version history first.
  if (hasDirectContent) {
    try {
      saveDocumentVersion(slug, doc.markdown ?? '', doc.title, doc.revision, `pre-rewrite:${by}`, doc.marks);
    } catch (e) {
      console.error('[canonical-document] Failed to save pre-rewrite version:', e);
    }
  }
  const nextMarks = hasDirectContent ? {} : parseMarks(doc.marks);
