...
type HocuspocusInstance = {
  ...
  // Available in @hocuspocus/server 2.x (Agentdoc uses 2.15.x).
...
const collabSigningSecret = (env('AGENTDOC_COLLAB_SIGNING_SECRET', 'PROOF_COLLAB_SIGNING_SECRET') || '').trim()
...
function resolveAttachedWsUrlBase(mainHttpPort: number): string {
  ...
  const publicBase = (env('AGENTDOC_PUBLIC_BASE_URL', 'PROOF_PUBLIC_BASE_URL') || '').trim().replace(/\/+$/, '');
  if (publicBase) {
    try {
...
async function refreshMarkdownTextFromFragment(
  ...
  if (derivedFragmentMarkdown !== currentMarkdown) {
    ...
    // Guard: if the current markdown contains GFM table pipes but the derived
    // markdown has lost them, the serializer failed to round-trip table nodes.
    // Treat this as a derive failure so we don't overwrite canonical content
    // with a lossy re-serialization.
    const currentHasTables = currentMarkdown.includes('|');
    const derivedHasTables = derivedFragmentMarkdown.includes('|');
    if (currentHasTables && !derivedHasTables) {
      console.warn('[collab] refusing to overwrite markdown text: derived fragment lost table content', {
        slug,
        currentChars: currentMarkdown.length,
        derivedChars: derivedFragmentMarkdown.length,
        sourceActor,
      });
      return { deriveFailed: true, refreshed: false, markdown: null };
    }
...
async function repairProjectionFromFragment(
  ...
  // Guard: if the canonical DB markdown contains GFM table pipes but the
  // derived fragment markdown does not, the serializer lost table content.
  // Use the canonical DB markdown instead to avoid silently dropping tables.
  if (row.markdown.includes('|') && !derivedMarkdown.includes('|')) {
    console.warn('[collab] projection repair: derived markdown lost table pipes; keeping canonical DB markdown', {
      slug,
      reasons,
      rowChars: row.markdown.length,
      derivedChars: derivedMarkdown.length,
    });
    recordProjectionRepair('failure', reasons.join('|') || 'derived_lost_table_content');
    return 'retry';
  }
...
function projectionPayloadMatchesCanonical(doc: ProjectedDocumentRow | null | undefined): boolean {
  ...
  // Fast path: if raw strings match, skip expensive parse+normalize+serialize
  if (doc.marks === canonicalMarks) return true;
...
export function stripEphemeralCollabSpans(markdown: string): string {
  ...
  const cursorSpanPattern = /<span\b[^>]*(?:ProseMirror-yjs-cursor|agentdoc-collab-cursor|agentdoc-collab-cursor|agentdoc-agent-cursor|agentdoc-agent-cursor|data-agentdoc-cursor|data-agentdoc-cursor|data-agent-cursor)[^>]*>[\s\S]*?<\/span>/gi;
...
export async function startCollabRuntime(mainHttpPort: number): Promise<CollabRuntime> {
  const flag = (env('AGENTDOC_COLLAB_V2', 'PROOF_COLLAB_V2') || '').trim().toLowerCase();
  const disabled = flag === '0' || flag === 'false' || flag === 'off' || flag === 'disabled';
  if (disabled) {
    runtime = {
      enabled: false,
      wsUrlBase: '',
      reason: 'Disabled by AGENTDOC_COLLAB_V2 flag',
    };
    return runtime;
  }
  ...
  const hasConfiguredCollabSecret = Boolean((env('AGENTDOC_COLLAB_SIGNING_SECRET', 'PROOF_COLLAB_SIGNING_SECRET') || '').trim());
  if (!hasConfiguredCollabSecret && !isLocalWsUrlBase(collabPublicBase)) {
    runtime = {
      enabled: false,
      wsUrlBase: '',
      reason: 'AGENTDOC_COLLAB_SIGNING_SECRET is required for non-local collab runtime',
    };
    return runtime;
  }

  try {
    if (!warnedAboutEphemeralCollabSecret && !(env('AGENTDOC_COLLAB_SIGNING_SECRET', 'PROOF_COLLAB_SIGNING_SECRET') || '').trim()) {
      warnedAboutEphemeralCollabSecret = true;
      console.warn('[collab] AGENTDOC_COLLAB_SIGNING_SECRET is not set; using ephemeral in-memory signing key');
    }
