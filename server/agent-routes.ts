...
import { getShareRole } from './mcp/sharing.js';
import {
...
const REWRITE_COLLAB_TIMEOUT_MS = parsePositiveInt(env('AGENTDOC_REWRITE_COLLAB_TIMEOUT_MS', 'PROOF_REWRITE_COLLAB_TIMEOUT_MS'), 3000);
const REWRITE_BARRIER_TIMEOUT_MS = parsePositiveInt(env('AGENTDOC_REWRITE_BARRIER_TIMEOUT_MS', 'PROOF_REWRITE_BARRIER_TIMEOUT_MS'), 5000);
...
function getPresentedSecret(req: Request, slug?: string | null): string | null {
  ...
  if (resolvedSlug) {
    const fromCookie = getCookie(req, shareTokenCookieName(resolvedSlug)) ?? getCookie(req, legacyShareTokenCookieName(resolvedSlug));
...
function shouldIncludeCanonicalDiagnostics(): boolean {
  const runtimeEnv = (env('AGENTDOC_ENV', 'PROOF_ENV') || process.env.NODE_ENV || '').trim().toLowerCase();
...
function getSessionEmail(req: Request): string | undefined {
  return (req.session as Record<string, any> | undefined)?.user?.email;
}

function resolveSessionRole(req: Request, slug: string): ShareRole | null {
...
function checkAuth(
  ...
  const resolvedRole = secret ? resolveDocumentAccessRole(slug, secret) : null;
  // Self-hosted: default to editor when no token is provided and document is active
  const selfHostedRole = env('AGENTDOC_SELF_HOSTED', 'PROOF_SELF_HOSTED') === '1' && doc.share_state === 'ACTIVE' ? 'editor' as ShareRole : null;
  const role = resolvedRole ?? resolveSessionRole(req, slug) ?? selfHostedRole;
  ...
  if (!hasRole(role, allowedRoles)) {
    // Distinguish unauthenticated (needs login) from insufficient permissions
    if (!secret && !getSessionEmail(req)) {
      res.status(401).json({
        success: false,
...
agentRoutes.get('/:slug/state', (req: Request, res: Response) => {
  ...
  const doc = getDocumentBySlug(slug);
  body.author = doc?.owner_id || '';
  if (doc?.created_at) body.createdAt = doc.created_at;
  if (doc?.updated_at) body.updatedAt = doc.updated_at;
  ...
  if (mutationReady) {
    links.ops = { method: 'POST', href: agentdocPaths.ops };
    links.edit = { method: 'POST', href: agentdocPaths.edit };
    links.title = { method: 'PUT', href: agentdocPaths.title };
  }
  if (editV2Enabled) {
    links.snapshot = agentdocPaths.snapshot;
    if (mutationReady) {
      links.editV2 = { method: 'POST', href: agentdocPaths.editV2 };
    } else {
      delete links.editV2;
    }
  }
  ...
  if (mutationReady) {
    agent.opsApi = agentdocPaths.ops;
    agent.editApi = agentdocPaths.edit;
    agent.titleApi = agentdocPaths.title;
  }
  if (editV2Enabled) {
    agent.snapshotApi = agentdocPaths.snapshot;
    if (mutationReady) {
      agent.editV2Api = agentdocPaths.editV2;
    }
  }
  body.agent = agent;

  // Strip Agentdoc-authored span tags from agent-facing markdown so agents see clean text.
  if (typeof body.markdown === 'string') {
    body.markdown = stripAgentdocSpanTags(body.markdown);
  }
  if (typeof body.content === 'string') {
    body.content = stripAgentdocSpanTags(body.content);
  }
...
agentRoutes.post('/:slug/edit', async (req: Request, res: Response) => {
  ...
  res.setHeader('X-Agentdoc-Agent-Routes', '1');
  res.setHeader('X-Agentdoc-Legacy-Edit-Hosted', hostedRuntime ? '1' : '0');
  res.setHeader('X-Agentdoc-Legacy-Edit-Collab', collabRuntime.enabled ? '1' : '0');
  res.setHeader('X-Agentdoc-Legacy-Edit-Clients', String(activeCollabClients));
  ...
  const responseBody = {
    ...
    agent: buildAgentdocAgentDescriptor(slug, {
      includeMutationRoutes: true,
    }),
  } satisfies Record<string, unknown>;
...
agentRoutes.post('/:slug/presence', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    slug,
    collabApplied,
    _links: buildAgentdocLinks(slug, {
      includeMutationRoutes: true,
    }),
    agent: buildAgentdocAgentDescriptor(slug, {
      includeMutationRoutes: true,
...
agentRoutes.post('/:slug/ops', async (req: Request, res: Response) => {
  ...
  const secret = getPresentedSecret(req, slug);
  const resolvedRole = secret ? resolveDocumentAccessRole(slug, secret) : null;
  const role = resolvedRole ?? (env('AGENTDOC_SELF_HOSTED', 'PROOF_SELF_HOSTED') === '1' && doc.share_state === 'ACTIVE' ? 'editor' as ShareRole : null);
...
agentRoutes.post('/:slug/rewrite', async (req: Request, res: Response) => {
  ...
  res.setHeader('X-Agentdoc-Agent-Routes', '1');
  res.setHeader('X-Agentdoc-Rewrite-Hosted', rewriteGate.hostedRuntime ? '1' : '0');
  res.setHeader('X-Agentdoc-Rewrite-Blocked', rewriteGate.blocked ? '1' : '0');
  res.setHeader('X-Agentdoc-Rewrite-Clients', String(rewriteGate.connectedClients));
  if (rewriteGate.blocked) {
    recordRewriteLiveClientBlock(
      mutationRoute,
      rewriteGate.runtimeEnvironment,
