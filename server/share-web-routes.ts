...
import { hasDocumentAccess } from './mcp/sharing.js';
...
import { stripAgentdocSpanTags } from './agentdoc-span-strip.js';
import {
...
/**
 * Rate limiter for magic token lookups: 10 attempts per IP per minute.
 * Prevents brute-force enumeration of magic share tokens.
 */
const magicTokenRateLimiter = createRateLimiter({
  windowMs: 60_000,
  maxRequests: 10,
  keyFn: (req: Request) => {
    const forwarded = req.header('x-forwarded-for');
    const ip = forwarded ? forwarded.split(',')[0].trim() : req.ip || 'unknown';
    return `magic-token:${ip}`;
  },
});
...
function getPublicOrigin(req: Request): string {
  const configured = env('AGENTDOC_PUBLIC_ORIGIN', 'PROOF_PUBLIC_ORIGIN')?.trim();
...
// SPA fallback: serve editor.html for /d/:slug routes
let shareHtml: string | null = null;
const editorHtmlPath = path.resolve(__dirname, '..', 'public', 'editor.html');
...
function isForcedLiveViewerRequest(req: Request): boolean {
  ...
  const liveHeader = (req.header('x-agentdoc-live-viewer') || '').trim().toLowerCase();
...
function buildShareRuntimeConfigScript(slug: string, shareToken?: string | null): string {
  const commentUiDefaultMode = normalizeCommentUiMode(env('AGENTDOC_COMMENT_UI_DEFAULT_MODE', 'PROOF_COMMENT_UI_DEFAULT_MODE'));
  const configLines = [
    `window.__AGENTDOC_CONFIG__.shareSlug = ${JSON.stringify(slug)};`,
    shareToken ? `window.__AGENTDOC_CONFIG__.shareToken = ${JSON.stringify(shareToken)};` : '',
    commentUiDefaultMode ? `window.__AGENTDOC_CONFIG__.commentUiDefaultMode = ${JSON.stringify(commentUiDefaultMode)};` : '',
  ].filter(Boolean);
  return `<script>
window.__AGENTDOC_CONFIG__ = window.__AGENTDOC_CONFIG__ || {};
${configLines.join('\n')}
</script>`;
}
...
function injectShareHtmlDiscoveryTags(
  ...
  const agentdocPaths = buildAgentdocDocumentPaths(slug);
  const agentApi = agentdocPaths.state;
  const editApi = agentdocPaths.edit;
  const opsApi = agentdocPaths.ops;
  const fullMetaTags = renderShareMetaTags(preview);
  const pageTitle = fullMetaTags.match(/<title>[\s\S]*?<\/title>/i)?.[0]
    ?? `<title>${escapeHtml(`${preview.title} | AgentDoc`)}</title>`;
  const metaTags = [
  ...
  const instructionMarkup = `<h2>AgentDoc Shared Document</h2>
  <p>This is a collaborative document on AgentDoc. To read or edit it programmatically:</p>
  <ul>
  ...
  <h3>Document Content</h3>
  <pre>${escapeHtml(markdown)}</pre>
  <h2>AgentDoc Shared Document</h2>
  ${instructionMarkup}
</div>`;
...
function renderAgentFriendlyHtml(
  ...
  const agentdocPaths = buildAgentdocDocumentPaths(slug, origin);
  const stateUrl = agentdocPaths.state;
  const editUrl = agentdocPaths.edit;
  const opsUrl = agentdocPaths.ops;
...
<body>
  <h1>AgentDoc Shared Document</h1>
  <p>This is a collaborative document on AgentDoc.</p>
...
shareWebRoutes.get('/d/:slug', (req: Request, res: Response, next) => {
  // Apply rate limiting only when a magic token is being resolved.
  if (typeof req.query.token === 'string' && req.query.token.trim()) {
    magicTokenRateLimiter(req, res, next);
    return;
  }
  next();
  ...
  // Check visibility before loading the full document
  if (slug) {
    ...
    const tokenFromQueryCheck = typeof req.query.token === 'string' ? req.query.token.trim() : '';
    const hasShareToken = tokenFromQueryCheck && resolveDocumentAccessRole(slug, tokenFromQueryCheck);
    ...
    if (visibility === 'private') {
      // Private: owner only
      if (ownerEmail && !isOwner && !hasShareToken) {
        const preview = buildSharePreviewModel({ slug, origin, shareState: 'MISSING' });
        if (wantsJson(req)) {
          res.status(404).json({ success: false, slug, error: 'Document not accessible' });
          return;
        }
        res.status(404).type('html').send(renderUnavailableHtml(preview, 'This document is private and not accessible.'));
        return;
      }
    }

    if (visibility === 'selective') {
      // Selective: owner + explicitly shared users only
      if (!isOwner && !hasPerUserAccess && !hasShareToken) {
        const preview = buildSharePreviewModel({ slug, origin, shareState: 'MISSING' });
        if (wantsJson(req)) {
          res.status(403).json({ success: false, slug, error: 'You do not have access to this document' });
          return;
        }
        res.status(403).type('html').send(renderUnavailableHtml(preview, 'You do not have access to this document. Ask the owner to share it with you.'));
        return;
      }
    }

    if (visibility === 'shared') {
      // Organization: owner + per-user shares + all domain users
      if (!isOwner && !hasShareToken && !hasPerUserAccess && !isDomainUser) {
        const preview = buildSharePreviewModel({ slug, origin, shareState: 'MISSING' });
        if (wantsJson(req)) {
          res.status(403).json({ success: false, slug, error: 'You do not have access to this document' });
          return;
    ...
    // Public: no access check needed
  }

  const doc = slug ? (getCanonicalReadableDocumentSync(slug, 'share') ?? null) : null;
  const tokenFromCookie = slug ? (getCookie(req, shareTokenCookieName(slug)) ?? getCookie(req, legacyShareTokenCookieName(slug))) : null;
  ...
  if (doc && doc.share_state !== 'ACTIVE' && !ownerOverride) {
    ...
    if (snapshot) {
      ...
      res.setHeader('x-agentdoc-fallback', 'snapshot');
  ...
  if (wantsMarkdown(req)) {
    ...
    res.type('text/markdown').send(stripAgentdocSpanTags(doc.markdown ?? ''));
    return;
  }
  ...
  if (wantsJson(req)) {
    ...
    const agent: Record<string, unknown> = {
      ...buildAgentdocAgentDescriptor(slug, {
        origin,
        includeMutationRoutes: mutationReady,
        includeSnapshotRoute: editV2Enabled,
        includeEditV2Route: editV2Enabled && mutationReady,
    ...
    res.json({
      ...
      markdown: doc?.markdown ? stripAgentdocSpanTags(doc.markdown) : null,
      readSource,
      projectionFresh,
      mutationReady,
      shareState,
  ...
  if (shouldReloadShareHtml) {
    try {
      shareHtml = readFileSync(editorHtmlPath, 'utf-8');
    } catch {
