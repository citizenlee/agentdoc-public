...
const LOGO_PATH = resolveRepoPath('public', 'assets', 'og-share', 'agentdoc-logo-outlined.svg');
...
export function buildSharePreviewModel(input: {
  ...
  const bodyText = truncate(
    isUnavailable ? buildUnavailableDescription(shareState) : (excerpt ?? 'Shared on AgentDoc'),
...
export function renderShareMetaTags(model: SharePreviewModel): string {
  const title = escapeHtml(`${model.title} | AgentDoc`);
  ...
  return [
    ...
    '<meta property="og:site_name" content="AgentDoc">',
