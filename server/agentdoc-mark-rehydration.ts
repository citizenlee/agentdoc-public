...
import {
  ...
  listAuthoredAgentdocSpanBounds,
  stripAllAgentdocSpanTags,
...
export type AgentdocMarkRehydrationFailureCode =
  | 'MARKDOWN_PARSE_FAILED'
  | 'MARK_NOT_HYDRATED'
  | 'REQUIRED_MARKS_MISSING'
  | 'STRUCTURED_MUTATION_FAILED';

export type AgentdocMarkRehydrationFailure = {
  ok: false;
  code: AgentdocMarkRehydrationFailureCode;
  error: string;
...
export type AgentdocMarkRehydrationSuccess = {
  ok: true;
  markdown: string;
...
export type AgentdocMarkRehydrationResult = AgentdocMarkRehydrationSuccess | AgentdocMarkRehydrationFailure;
...
function buildSerializedAuthoredFingerprintCounts(markdown: string): Map<string, number> {
  ...
  for (const span of listAuthoredAgentdocSpanBounds(markdown)) {
    ...
    const quote = normalizeQuote(stripHtmlTags(stripAllAgentdocSpanTags(inner)));
    if (!quote) continue;
    const key = `${by}::${quote}`;
...
async function buildRehydratedState(markdown: string, marks: Record<string, StoredMark>): Promise<{
...
} | AgentdocMarkRehydrationFailure> {
  ...
  const effectiveMarks = serializedAuthoredMarks
    ...
    : marks;
  const strippedMarkdown = stripAllAgentdocSpanTagsWithReplacements(
    markdown ?? '',
    buildAgentdocSpanReplacementMap(effectiveMarks),
  );
  ...
  if (!parsed.doc) {
    return {
      ...
      error: 'Failed to parse markdown after stripping Agentdoc spans',
      strippedMarkdown,
      hydratedMarkIds: [],
...
async function finalizeRehydratedState(
  strippedMarkdown: string,
  state: EditorState,
): Promise<AgentdocMarkRehydrationSuccess> {
  ...
  return {
    ...
    marks: repairedMarks,
    strippedMarkdown,
    repairedStrippedMarkdown: stripAllAgentdocSpanTags(repairedMarkdown),
    hydratedMarkIds,
    missingRequiredMarkIds,
  };
}

export async function rehydrateAgentdocMarksMarkdown(
  markdown: string,
  marks: Record<string, StoredMark>,
): Promise<AgentdocMarkRehydrationResult> {
  ...
  if (rehydrated.missingRequiredIds.length > 0) {
    return missingMarkFailure(
      'REQUIRED_MARKS_MISSING',
      'One or more stored Agentdoc marks could not be rehydrated safely',
...
export async function finalizeSuggestionThroughRehydration(args: {
  ...
  if (!rehydrated.hydratedIds.includes(args.markId)) {
    return missingMarkFailure(
      'MARK_NOT_HYDRATED',
      'Target Agentdoc mark could not be rehydrated from stored anchors',
      rehydrated.strippedMarkdown,
      rehydrated.hydratedIds,
      rehydrated.missingRequiredIds,
    );
