...
import { getShareRole, hasDocumentAccess } from './mcp/sharing.js';
import {
...
import { getCookie, shareTokenCookieName, legacyShareTokenCookieName } from './cookies.js';
...
import {
  buildAgentdocAgentDescriptor,
  buildAgentdocDocumentPaths,
  buildAgentdocLinks,
} from './agentdoc-sdk-routes.js';

export const apiRoutes = Router();
runLegacyMarkRangeBackfillOnce();

// One-time migration: clear all Yjs collab state after proof→agentdoc rename.
// ProseMirror mark type names changed (proofSuggestion→agentdocSuggestion etc.)
// so stored Yjs state is incompatible. Clearing forces re-seed from canonical markdown.
{
  const RENAME_MIGRATION_KEY = 'proof-to-agentdoc-yjs-reset-2026-03-23';
  if (!hasMaintenanceRun(RENAME_MIGRATION_KEY)) {
    const d = getDb();
    const cleared = d.prepare('DELETE FROM document_y_snapshots').run().changes;
    const updates = d.prepare('DELETE FROM document_y_updates').run().changes;
    d.prepare('UPDATE documents SET y_state_version = 0 WHERE share_state != \'DELETED\'').run();
    d.prepare('UPDATE document_projections SET y_state_version = 0').run();
    recordMaintenanceRun(RENAME_MIGRATION_KEY, { cleared, updates });
    console.log('[migration] Cleared Yjs state for proof→agentdoc rename', { cleared, updates });
  }
}
...
const OPS_RATE_LIMIT_WINDOW_MS = parsePositiveIntEnv('AGENTDOC_OPS_RATE_LIMIT_WINDOW_MS', 60_000, 'PROOF_OPS_RATE_LIMIT_WINDOW_MS');
const OPS_RATE_LIMIT_MAX_REQUESTS = parsePositiveIntEnv('AGENTDOC_OPS_RATE_LIMIT_MAX', 120, 'PROOF_OPS_RATE_LIMIT_MAX');
...
function getDirectShareApiKey(): string | null {
  const key = (env('AGENTDOC_SHARE_MARKDOWN_API_KEY', 'PROOF_SHARE_MARKDOWN_API_KEY') || '').trim();
...
function checkDirectShareRateLimit(
  ...
  const max = authed
    ? parsePositiveIntEnv('AGENTDOC_SHARE_MARKDOWN_RATE_LIMIT_MAX_AUTH_PER_MIN', DEFAULT_DIRECT_SHARE_RATE_LIMIT_MAX_AUTH_PER_MIN, 'PROOF_SHARE_MARKDOWN_RATE_LIMIT_MAX_AUTH_PER_MIN')
    : parsePositiveIntEnv('AGENTDOC_SHARE_MARKDOWN_RATE_LIMIT_MAX_UNAUTH_PER_MIN', DEFAULT_DIRECT_SHARE_RATE_LIMIT_MAX_UNAUTH_PER_MIN, 'PROOF_SHARE_MARKDOWN_RATE_LIMIT_MAX_UNAUTH_PER_MIN');
  const windowMs = parsePositiveIntEnv('AGENTDOC_SHARE_MARKDOWN_RATE_LIMIT_WINDOW_MS', DEFAULT_DIRECT_SHARE_RATE_LIMIT_WINDOW_MS, 'PROOF_SHARE_MARKDOWN_RATE_LIMIT_WINDOW_MS');
  const now = Date.now();
...
function getExplicitShareSecret(req: Request): string | null {
  ...
  if (typeof slugParam === 'string' && slugParam.trim()) {
    const fromCookie = getCookie(req, shareTokenCookieName(slugParam.trim())) ?? getCookie(req, legacyShareTokenCookieName(slugParam.trim()));
...
function getAccessRole(req: Request, slug: string): ShareRole | null {
  ...
  // Session-based auth: logged-in users get role from per-user shares or domain membership
  const sessionEmail = req.session?.user?.email;
  if (sessionEmail) {
    // Check if user is the document owner
    const ownerEmail = getDocumentOwnerEmail(slug);
    if (ownerEmail && ownerEmail === sessionEmail) return 'owner_bot';

    // Check per-user share role
    const shareRole = getShareRole(slug, sessionEmail);
    if (shareRole && isShareRole(shareRole)) return shareRole;

    // Domain users get editor access on shared/public documents
    ...
    if (visibility === 'shared' || visibility === 'public') {
      ...
      if (emailDomain === allowedDomain) return 'editor';
    }
  }

  // Security: tokenless requests default to viewer (read-only) to prevent
  // unauthorized edits. Explicit share tokens are required for editor access.
  return 'viewer';
}
...
async function resolveOpenContextAccess(
  ...
  // Security: tokenless requests default to viewer (read-only) to prevent
  // unauthorized edits. Explicit share tokens are required for editor access.
  return { role: 'viewer', tokenId: null, ownerAuthorized: false };
}
...
apiRoutes.post('/documents', (req: Request, res: Response) => {
  ...
  // Auth check: require session, bearer token, or API key
  const hasSession = !!req.session?.user?.email;
  const hasBearer = !!req.headers.authorization;
  const hasApiKey = !!req.headers['x-share-token'];
  if (!hasSession && !hasBearer && !hasApiKey) {
    res.status(401).json({ error: 'Authentication required to create documents' });
    return;
  }

  const { markdown, marks, title, ownerId, ownerEmail: bodyOwnerEmail } = req.body;
  ...
  // Store owner email for visibility access control.
  // Prefer session email (browser), fall back to ownerEmail from request body (API/Claude Code).
  const effectiveOwnerEmail = req.session?.user?.email || (typeof bodyOwnerEmail === 'string' ? bodyOwnerEmail.trim() : '');
  if (effectiveOwnerEmail) {
    setDocumentOwnerEmail(slug, effectiveOwnerEmail);
  }
  ...
  res.json({
    ...
    _links: {
      ...
      web: links.shareUrl,
      tokenUrl: shareUrlWithToken,
      ...buildAgentdocLinks(doc.slug, {
        includeMutationRoutes: true,
        includeBridgeRoutes: true,
      }),
    },
    agent: buildAgentdocAgentDescriptor(doc.slug, {
      includeMutationRoutes: true,
      includeBridgeRoutes: true,
    }),
...
export async function handleShareMarkdown(req: Request, res: Response): Promise<void> {
  ...
  const agentdocPaths = buildAgentdocDocumentPaths(doc.slug);
  refreshSnapshotForSlug(slug);
  ...
  res.json({
    ...
    _links: {
      ...
      comment: {
        method: 'POST',
        href: agentdocPaths.bridgeComments,
        body: { quote: '...', text: '...', by: 'ai:your-agent' },
      },
      suggest: {
        method: 'POST',
        href: agentdocPaths.bridgeSuggestions,
        body: { kind: 'replace', quote: '...', content: '...', by: 'ai:your-agent' },
      },
      rewrite: {
        method: 'POST',
        href: agentdocPaths.bridgeRewrite,
...
// List all active documents (lightweight dashboard)
apiRoutes.get('/documents', (req: Request, res: Response) => {
  try {
    const docs = listActiveDocumentsMeta();
    const sessionEmail = req.session?.user?.email || null;

    // Batch queries: slugs with shares + last viewed times (2 queries instead of 2N)
    const slugs = docs.map(d => d.slug);
    const db = getDb();
    const sharesSet = new Set(
      slugs.length > 0
        ? (db.prepare(
            `SELECT DISTINCT document_slug FROM document_shares WHERE document_slug IN (${slugs.map(() => '?').join(',')})`
          ).all(...slugs) as Array<{ document_slug: string }>).map(r => r.document_slug)
        : []
    );
    const lastViewedMap = new Map(
      slugs.length > 0
        ? (db.prepare(
            `SELECT document_slug, MAX(viewed_at) as last_viewed FROM document_views WHERE document_slug IN (${slugs.map(() => '?').join(',')}) GROUP BY document_slug`
          ).all(...slugs) as Array<{ document_slug: string; last_viewed: string }>).map(r => [r.document_slug, r.last_viewed])
        : []
    );

    const mapped = docs.map((d) => {
      const visibility = d.visibility || 'private';
      const ownerEmail = d.owner_email || null;
      if (visibility === 'private') {
        if (!sessionEmail) return null;
        if (ownerEmail && ownerEmail !== sessionEmail) return null;
      }
      return {
        slug: d.slug,
        ...
        lastViewedAt: lastViewedMap.get(d.slug) || null,
        archived: d.archived === 1,
        hasShares: sharesSet.has(d.slug),
      };
    }).filter(Boolean)
    .sort((a, b) => ((b as any).updatedAt || '').localeCompare((a as any).updatedAt || ''));
    res.json(mapped);
  } catch (e) {
...
apiRoutes.get('/documents/:slug', (req: Request, res: Response) => {
  ...
  if (!ownerOverride) {
    ...
    const sessionEmail = req.session?.user?.email;
    const ownerEmail = getDocumentOwnerEmail(slug);
    const isOwner = ownerEmail && sessionEmail && sessionEmail === ownerEmail;
    const tokenHeader = typeof req.headers['x-share-token'] === 'string' ? req.headers['x-share-token'] : '';
    const hasToken = tokenHeader && resolveDocumentAccessRole(slug, tokenHeader);
    const hasPerUserAccess = sessionEmail && hasDocumentAccess(slug, sessionEmail);
    const allowedDomain = process.env.GOOGLE_ALLOWED_DOMAIN || 'cultivarium.org';
    const isDomainUser = sessionEmail && sessionEmail.split('@')[1] === allowedDomain;
    ...
    } else if (visibility === 'selective') {
      if (!isOwner && !hasPerUserAccess && !hasToken) {
        res.status(403).json({ error: 'You do not have access to this document' });
        return;
      }
    } else if (visibility === 'shared') {
      if (!isOwner && !hasToken && !hasPerUserAccess && !isDomainUser) {
        res.status(403).json({ error: 'You do not have access to this document' });
        return;
      }
    }
  ...
  res.json({
    ...
    _links: buildAgentdocLinks(doc.slug, {
      includeMutationRoutes: true,
...
apiRoutes.put('/documents/:slug/title', (req: Request, res: Response) => {
  ...
  const isLoggedInDomainUser = !!sessionEmail;
  const visibility = getDocumentVisibility(slug);
  // Allow: owner, token with editor/owner_bot, logged-in domain user on shared/public/unclaimed docs
  const canEditTitle = ownerOrBot || isOwner
    || (doc.share_state === 'ACTIVE' && accessRole === 'editor')
    || (isLoggedInDomainUser && (visibility === 'shared' || visibility === 'public'))
    || (isLoggedInDomainUser && !ownerEmail);
...
apiRoutes.put('/documents/:slug', async (req: Request, res: Response) => {
  ...
  if (accessRole && accessRole !== 'viewer') {
    const putOp = hasMarkdownUpdate ? 'markdown' : hasMarksUpdate ? 'marks' : 'title';
    addDocumentEvent(slug, 'token.write', { role: accessRole, op: putOp }, req.session?.user?.email || 'token-user');
  }
  ...
  res.json({
    ...
    updatedAt: updatedDoc.updated_at,
    _links: buildAgentdocLinks(updatedDoc.slug, {
      includeMutationRoutes: true,
      includeSnapshotRoute: true,
      includeEditV2Route: true,
      includeBridgeRoutes: true,
    }),
    agent: buildAgentdocAgentDescriptor(updatedDoc.slug, {
      includeMutationRoutes: true,
      includeSnapshotRoute: true,
      includeEditV2Route: true,
      includeBridgeRoutes: true,
...
apiRoutes.post('/documents/:slug/ops', opsRateLimiter, async (req: Request, res: Response) => {
  ...
  if (denied) {
    if (denied === 'Insufficient role for operation' && !req.session?.user) {
  ...
  if (accessRole && accessRole !== 'viewer') {
    addDocumentEvent(slug, 'token.write', { role: accessRole, op }, req.session?.user?.email || 'token-user');
  }

  if (idempotencyKey) {
    const userIdentity = req.session?.user?.email || req.headers['x-share-token'] || 'anonymous';
    const existing = getStoredIdempotencyRecord(slug, routeKey, idempotencyKey, userIdentity as string);
    if (existing) {
  ...
  if (idempotencyKey && result.status >= 200 && result.status < 300) {
    const storeUserIdentity = req.session?.user?.email || req.headers['x-share-token'] || 'anonymous';
    storeIdempotencyResult(slug, routeKey, idempotencyKey, result.body, requestHash, { statusCode: result.status, userIdentity: storeUserIdentity as string });
  }
