...
export function shareTokenCookieName(slug: string): string {
  ...
  return `agentdoc_share_token_${slug}`;
}

export function legacyShareTokenCookieName(slug: string): string {
...
export function ownerTokenCookieName(slug: string): string {
  ...
  return `agentdoc_owner_token_${slug}`;
}

export function legacyOwnerTokenCookieName(slug: string): string {
  return `proof_owner_token_${slug}`;
}

// ── Global session cookie (dashboard auth) ────────────────────────────────────

export const SESSION_COOKIE_NAME = 'agentdoc_session';
export const LEGACY_SESSION_COOKIE_NAME = 'proof_session';
const DEFAULT_SESSION_MAX_AGE_SEC = 30 * 24 * 60 * 60; // 30 days
const DASHBOARD_AUTH_COOKIE_PREFIX = 'agentdoc_dashboard_auth_';
const LEGACY_DASHBOARD_AUTH_COOKIE_PREFIX = 'proof_dashboard_auth_';
...
function getCookieDomain(req: Request): string | null {
  ...
  if (host === 'agentdoc.up.railway.app' || host === 'www.agentdoc.up.railway.app') {
    return '.agentdoc.up.railway.app';
  }
  return null;
}

export function getSessionCookie(req: Request): string | null {
  return getCookie(req, SESSION_COOKIE_NAME) ?? getCookie(req, LEGACY_SESSION_COOKIE_NAME);
}
...
function legacyDashboardAuthCookieName(state: string): string {
  return `${LEGACY_DASHBOARD_AUTH_COOKIE_PREFIX}${state}`;
}

export function getDashboardAuthReturnCookie(req: Request, state: string): string | null {
  return getCookie(req, dashboardAuthCookieName(state)) ?? getCookie(req, legacyDashboardAuthCookieName(state));
}
...
export function getOwnerTokenCookie(req: Request, slug: string): string | null {
  return getCookie(req, ownerTokenCookieName(slug)) ?? getCookie(req, legacyOwnerTokenCookieName(slug));
}
