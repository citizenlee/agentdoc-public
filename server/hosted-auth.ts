...
export function resolveShareMarkdownAuthMode(_publicBaseUrl?: string): Exclude<ShareMarkdownAuthMode, 'auto'> {
  const configured = (env('AGENTDOC_SHARE_MARKDOWN_AUTH_MODE', 'PROOF_SHARE_MARKDOWN_AUTH_MODE') || 'none').trim().toLowerCase();
...
export function startOAuthFlow(_publicBaseUrl: string):
  ...
  return {
    ok: false,
    error: 'OAuth is not available in Agentdoc SDK. Use share tokens or PROOF_SHARE_MARKDOWN_API_KEY.',
  };
}
...
export function pollOAuthFlow(
  ...
  return {
    status: 'failed',
    error: 'OAuth is not available in Agentdoc SDK.',
  };
}
...
export async function handleOAuthCallback(_input: {
  ...
  return {
    ok: false,
    message: 'OAuth is not available in Agentdoc SDK.',
  };
}
