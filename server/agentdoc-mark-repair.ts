...
import { rehydrateAgentdocMarksMarkdown } from './agentdoc-mark-rehydration.js';
...
export type AgentdocMarkRepairReport = {
  slug: string;
  safeToWrite: boolean;
  wrote: boolean;
  textStable: boolean;
...
async function repairDocumentRow(
...
): Promise<AgentdocMarkRepairReport> {
  const originalMarks = parseMarks(doc.marks);
  const repaired = await rehydrateAgentdocMarksMarkdown(doc.markdown, originalMarks);
  if (!repaired.ok) {
    return {
      slug: doc.slug,
      safeToWrite: false,
  ...
  const originalBaseMarkdown = stripAllAgentdocSpanTagsWithReplacements(
    doc.markdown,
    buildAgentdocSpanReplacementMap(originalMarks),
  );
...
export async function repairAgentdocMarksForSlug(
  slug: string,
  options?: { write?: boolean },
): Promise<AgentdocMarkRepairReport> {
...
export async function repairAgentdocMarksForAllDocuments(
  options?: { write?: boolean },
): Promise<AgentdocMarkRepairReport[]> {
  const docs = listActiveDocuments();
  const reports: AgentdocMarkRepairReport[] = [];
