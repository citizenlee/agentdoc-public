...
function getRuntimeEnvironment(): string {
  return normalizeEnvironment(env('AGENTDOC_ENV', 'PROOF_ENV') || process.env.NODE_ENV || 'development');
}
...
function readOrInitializeDatabaseEnvironment(d: Database.Database): string {
  ...
  const explicitInitEnvRaw = (env('AGENTDOC_DB_ENV_INIT', 'PROOF_DB_ENV_INIT') || '').trim();
...
function addMissingDocumentColumns(): void {
  ...
  if (!names.has('is_public')) {
    d.exec('ALTER TABLE documents ADD COLUMN is_public INTEGER NOT NULL DEFAULT 0');
  }
  if (!names.has('visibility')) {
    d.exec("ALTER TABLE documents ADD COLUMN visibility TEXT NOT NULL DEFAULT 'private'");
  }
  if (!names.has('owner_email')) {
    d.exec('ALTER TABLE documents ADD COLUMN owner_email TEXT');
  }
  if (!names.has('archived')) {
    d.exec('ALTER TABLE documents ADD COLUMN archived INTEGER NOT NULL DEFAULT 0');
  }
}
...
function addMissingMutationIdempotencyColumns(): void {
  ...
  if (!names.has('user_identity')) {
    d.exec(`ALTER TABLE ${MUTATION_IDEMPOTENCY_TABLE} ADD COLUMN user_identity TEXT`);
  }
...
function backfillDocumentColumns(): void {
  const d = getDb();
  const rows = d.prepare(`
    SELECT slug, doc_id, active, share_state, owner_secret, owner_secret_hash, revision, y_state_version, is_public, visibility, owner_id, owner_email
    FROM documents
  `).all() as Array<{
    slug: string;
    doc_id: string | null;
  ...
  const setVisibility = d.prepare('UPDATE documents SET visibility = ? WHERE slug = ?');
  const setOwnerEmail = d.prepare('UPDATE documents SET owner_email = ? WHERE slug = ?');
  ...
  for (const row of rows) {
    ...
    // Backfill visibility from is_public for existing rows
    if (!row.visibility || row.visibility === 'shared') {
      if (row.is_public === 1) {
        setVisibility.run('public', row.slug);
      }
    }
    // Backfill owner_email from owner_id for known users
    if (!row.owner_email && row.owner_id) {
      const oid = row.owner_id.toLowerCase();
      if (oid.includes('henry')) {
        setOwnerEmail.run('henry@cultivarium.org', row.slug);
      } else if (oid.includes('charlie')) {
        setOwnerEmail.run('charlie@cultivarium.org', row.slug);
      }
    }
  }
...
function ensureViewTrackingTable(): void {
  const d = getDb();
  d.exec(`
  ...
  `);
  d.exec('CREATE INDEX IF NOT EXISTS idx_document_views_slug ON document_views(document_slug)');
  d.exec('CREATE INDEX IF NOT EXISTS idx_document_views_slug_time ON document_views(document_slug, viewed_at)');

  // Add view_count column to documents if missing
  const cols = d.prepare("PRAGMA table_info('documents')").all() as Array<{ name: string }>;
  const names = new Set(cols.map((c) => c.name));
  if (!names.has('view_count')) {
    d.exec('ALTER TABLE documents ADD COLUMN view_count INTEGER NOT NULL DEFAULT 0');
  }
}

export function recordDocumentView(slug: string, email: string | null, name: string | null): void {
  const d = getDb();
  const now = new Date().toISOString();
  const userKey = email || name || 'Anonymous';
  // Only count if this user hasn't visited in the last hour
  const oneHourAgo = new Date(Date.now() - 60 * 60 * 1000).toISOString();
  const recent = d.prepare(`
    SELECT id FROM document_views
    WHERE document_slug = ? AND COALESCE(user_email, user_name) = ? AND viewed_at > ?
    LIMIT 1
  `).get(slug, userKey, oneHourAgo) as { id: number } | undefined;
  if (recent) return;
  d.prepare('INSERT INTO document_views (document_slug, user_email, user_name, viewed_at) VALUES (?, ?, ?, ?)').run(slug, email, name || 'Anonymous', now);
  d.prepare('UPDATE documents SET view_count = view_count + 1 WHERE slug = ?').run(slug);
}

export function getDocumentViewCount(slug: string): number {
  const row = getDb().prepare('SELECT view_count FROM documents WHERE slug = ? LIMIT 1').get(slug) as { view_count?: number } | undefined;
...
export function getDocumentViewHistory(slug: string, limit: number = 100): DocumentViewRow[] {
  return getDb().prepare('SELECT user_email, user_name, viewed_at FROM document_views WHERE document_slug = ? ORDER BY viewed_at DESC LIMIT ?').all(slug, limit) as DocumentViewRow[];
}

export function getDocumentRecentViewers(slug: string): Array<{ user_name: string; user_email: string | null }> {
  return getDb().prepare(`
    SELECT user_name, user_email, MAX(viewed_at) as last_view
    FROM document_views
    WHERE document_slug = ?
    GROUP BY COALESCE(user_email, user_name)
    ORDER BY last_view DESC
    LIMIT 10
  `).all(slug) as Array<{ user_name: string; user_email: string | null }>;
}
...
function initDatabase(): void {
  ...
  `);
  // Add marks_json column if missing (for existing tables)
  {
    const vcols = d.prepare("PRAGMA table_info('document_versions')").all() as Array<{ name: string }>;
    if (!vcols.some(c => c.name === 'marks_json')) {
      d.exec('ALTER TABLE document_versions ADD COLUMN marks_json TEXT');
    }
  }
  d.exec('CREATE INDEX IF NOT EXISTS idx_document_versions_slug ON document_versions(document_slug, id DESC)');
...
export function createDocument(
  ...
  d.prepare(`
    INSERT INTO documents (
      ...
      owner_id, owner_secret, owner_secret_hash, created_at, updated_at, deleted_at, visibility
    )
    VALUES (?, ?, ?, ?, ?, 1, 0, 'ACTIVE', 0, 1, ?, NULL, ?, ?, ?, NULL, 'private')
  `).run(
    slug,
    docId,
    title || null,
...
/** Lightweight list for dashboard — skips large markdown/marks columns */
export function listActiveDocumentsMeta(): Array<{
  slug: string; title: string | null; owner_id: string | null;
  created_at: string; updated_at: string; visibility: string;
  owner_email: string | null; archived: number; view_count: number;
}> {
  return getDb()
    .prepare(`SELECT slug, title, owner_id, created_at, updated_at,
      COALESCE(visibility, 'private') AS visibility, owner_email,
      COALESCE(archived, 0) AS archived, COALESCE(view_count, 0) AS view_count
      FROM documents WHERE share_state = 'ACTIVE'`)
    .all() as any;
}
...
export function setDocumentPublic(slug: string, isPublic: boolean): boolean {
  return setDocumentVisibility(slug, isPublic ? 'public' : 'shared');
}

export function isDocumentPublic(slug: string): boolean {
  return getDocumentVisibility(slug) === 'public';
}

export type DocumentVisibility = 'private' | 'selective' | 'shared' | 'public';

export function getDocumentVisibility(slug: string): DocumentVisibility {
  const row = getDb().prepare(`SELECT visibility FROM documents WHERE slug = ? LIMIT 1`).get(slug) as { visibility?: string } | undefined;
  const v = row?.visibility;
  if (v === 'private' || v === 'selective' || v === 'shared' || v === 'public') return v;
  return 'private';
}

export function setDocumentVisibility(slug: string, visibility: DocumentVisibility): boolean {
  assertWritesAllowed('setDocumentVisibility');
  const now = new Date().toISOString();
  const isPublic = visibility === 'public' ? 1 : 0;
  const result = getDb().prepare(`
    UPDATE documents SET visibility = ?, is_public = ?, updated_at = ? WHERE slug = ?
  `).run(visibility, isPublic, now, slug);
  return result.changes > 0;
}

export function getDocumentOwnerEmail(slug: string): string | null {
  const row = getDb().prepare(`SELECT owner_email FROM documents WHERE slug = ? LIMIT 1`).get(slug) as { owner_email?: string } | undefined;
  return row?.owner_email || null;
}

export function setDocumentOwnerEmail(slug: string, email: string): boolean {
  assertWritesAllowed('setDocumentOwnerEmail');
  const result = getDb().prepare(`
    UPDATE documents SET owner_email = ? WHERE slug = ?
  `).run(email, slug);
  return result.changes > 0;
}

export function isDocumentArchived(slug: string): boolean {
  const row = getDb().prepare('SELECT archived FROM documents WHERE slug = ? LIMIT 1').get(slug) as { archived?: number } | undefined;
  return row?.archived === 1;
}

export function setDocumentArchived(slug: string, archived: boolean): boolean {
  assertWritesAllowed('setDocumentArchived');
  const now = new Date().toISOString();
  const result = getDb().prepare('UPDATE documents SET archived = ?, updated_at = ? WHERE slug = ?').run(archived ? 1 : 0, now, slug);
  return result.changes > 0;
}

export function saveDocumentVersion(slug: string, markdown: string, title: string | null, revision: number | null, actor: string | null, marksJson?: string | null): void {
  assertWritesAllowed('saveDocumentVersion');
  const now = new Date().toISOString();
  getDb().prepare(`
    INSERT INTO document_versions (document_slug, revision, markdown, marks_json, title, actor, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(slug, revision, markdown, marksJson ?? null, title, actor, now);
}

export function listDocumentVersions(slug: string, limit = 50): Array<{
  id: number; revision: number | null; title: string | null; actor: string | null; created_at: string; markdown_length: number; has_comments: boolean;
}> {
  return getDb().prepare(`
    SELECT id, revision, title, actor, created_at, LENGTH(markdown) as markdown_length,
      CASE WHEN marks_json IS NOT NULL AND marks_json != '{}' AND marks_json != '' THEN 1 ELSE 0 END as has_comments
...
export function getDocumentVersion(slug: string, versionId: number): { id: number; markdown: string; marks_json: string | null; title: string | null; revision: number | null; actor: string | null; created_at: string } | null {
  return (getDb().prepare(`
    SELECT id, markdown, marks_json, title, revision, actor, created_at
    FROM document_versions
    WHERE document_slug = ? AND id = ?
    LIMIT 1
  `).get(slug, versionId) as { id: number; markdown: string; marks_json: string | null; title: string | null; revision: number | null; actor: string | null; created_at: string } | undefined) ?? null;
}
...
export function getStoredIdempotencyRecord(
  ...
  const d = getDb();
  const coordinatorRow = userIdentity
    ? d.prepare(`
        SELECT response_json, request_hash
        FROM ${MUTATION_IDEMPOTENCY_TABLE}
        WHERE idempotency_key = ? AND document_slug = ? AND route = ? AND user_identity = ?
        LIMIT 1
      `).get(idempotencyKey, documentSlug, route, userIdentity) as { response_json?: string; request_hash?: string | null } | undefined
    : d.prepare(`
        SELECT response_json, request_hash
        FROM ${MUTATION_IDEMPOTENCY_TABLE}
        WHERE idempotency_key = ? AND document_slug = ? AND route = ?
...
export function storeIdempotencyResult(
  ...
  options?: { statusCode?: number; tombstoneRevision?: number | null; userIdentity?: string },
): void {
  ...
  const userIdentity = options?.userIdentity ?? null;
  ...
  const tx = d.transaction(() => {
    ...
    d.prepare(`
      INSERT OR REPLACE INTO ${MUTATION_IDEMPOTENCY_TABLE} (
        idempotency_key, document_slug, route, response_json, request_hash, status_code, tombstone_revision, created_at, user_identity
      )
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      idempotencyKey,
