...
export function getRewriteRuntimeEnvironment(): string {
  const explicit = normalizeEnvironment(env('AGENTDOC_ENV', 'PROOF_ENV') || process.env.NODE_ENV);
...
export function isHostedRewriteEnvironment(runtimeEnvironment: string = getRewriteRuntimeEnvironment()): boolean {
  // Self-hosted: disable the conservative hosted block so the lightweight editor can save
  if (env('AGENTDOC_SELF_HOSTED', 'PROOF_SELF_HOSTED') === '1') return false;
